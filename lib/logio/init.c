#pragma ident "$Id: init.c,v 1.12 2016/08/01 22:07:16 dechavez Exp $"
/*======================================================================
 *
...
{
    if (strncasecmp(spec, LOGIO_SYSLOGD_PREFIX, strlen(LOGIO_SYSLOGD_PREFIX)) == 0) {
        return UseSyslog(lp, spec+1+strlen(LOGIO_SYSLOGD_PREFIX));
    } else if (strncasecmp(spec, LOGIO_SYSLOG_PREFIX, strlen(LOGIO_SYSLOG_PREFIX)) == 0) {
        return UseSyslog(lp, spec+1+strlen(LOGIO_SYSLOG_PREFIX));
    } else {
 ...
 *
 * $Log: init.c,v $
 * Revision 1.12  2016/08/01 22:07:16  dechavez
 * use LOGIO_SYSLOG_PREFIX and LOGIO_SYSLOGD_PREFIX (from logio.h) instead of
 * the (formerly defined here) SYSLOG_PREFIX and SYSLOGD_PREFIX
 *
 * Revision 1.11  2015/12/18 17:05:45  dechavez
 * changed default facility to local0
 *
