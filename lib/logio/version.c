#pragma ident "$Id: version.c,v 1.25 2016/08/01 22:07:41 dechavez Exp $"
/*======================================================================
 * 
...
static VERSION version = {2, 4, 3};

/* logio library release notes

2.4.3   08/01/2016
        logio.h: define LOGIO_SYSLOG_PREFIX and LOGIO_SYSLOGD_PREFIX
        init.c: use LOGIO_SYSLOG_PREFIX and LOGIO_SYSLOGD_PREFIX (from logio.h)
                instead of SYSLOG_PREFIX and SYSLOGD_PREFIX (formerly locally defined)

2.4.2   11/13/2015
        msg.c: added explicit format string to syslog() to calm Darwin compiles
        watchdog.c: added some casts and printf() format string changes to calm Darwin compiles
