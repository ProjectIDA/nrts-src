#pragma ident "$Id: watchdog.c,v 1.5 2015/11/13 22:12:49 dechavez Exp $"
/*======================================================================
 *
 * Watchdog logging.
 *
...
char path[MAXPATHLEN+1];
    ...
    sprintf(path, "%s/%lu", watch->dir, (unsigned long) THREAD_SELF());
    if ((fp = fopen(path, watch->newline ? "a" : "w")) == NULL) return;
...
LOGIO_WATCHDOG *watch;
    ...
    sprintf(watch->dir, "%s/%s.%d", dir, name, getpid());
 ...
 *
 * $Log: watchdog.c,v $
 * Revision 1.5  2015/11/13 22:12:49  dechavez
 * added some casts and printf() format string changes to calm Darwin compiles
 *
 * Revision 1.4  2014/04/14 18:55:10  dechavez
 * return NULL; -> return; in void logioUpdateWatchdog()
 *
