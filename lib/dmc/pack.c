#pragma ident "$Id: pack.c,v 1.1 2014/07/10 20:30:42 dechavez Exp $"
/***************************************************************************
 * pack.c:
 *
	   ...
	   void *handlerdata, int64_t *packedsamples, flag flush, flag verbose )
{
  ...
  if ( ! msr->Blkt1000 )
	...
	ms_log (1, "%s: Adding 1000 Blockette\n", PACK_SRCNAME);
      ...
      if ( ! msr_addblockette (msr, (char *) &blkt1000, sizeof(struct blkt_1000_s), 1000, 1) )
	{
 ...
 *
 * Revision 1.3  2014/04/25 17:20:15  dechavez
 * make blockette 1000 the first in the chain (changed chain_pos argument from 0 to 1)
 *
