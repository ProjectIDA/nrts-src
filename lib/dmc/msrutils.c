#pragma ident "$Id: msrutils.c,v 1.1 2014/07/10 20:30:42 dechavez Exp $"
/***************************************************************************
 * msrutils.c:
 *
...
msr_normalize_header ( MSRecord *msr, flag verbose )
{
  ...
  if ( msr->fsdh )
	...
	msr->sequence_number = 1;
      ...
      msr->fsdh->act_flags = msr->flags.act_flags;
      msr->fsdh->io_flags  = msr->flags.io_flags;
      msr->fsdh->dq_flags  = msr->flags.dq_flags;
  ...
  while ( cur_blkt )
      ...
      else if ( cur_blkt->blkt_type == 1001 )
	{
	  ...
	  if (msr->flags.timing_qual >= 0) msr->Blkt1001->timing_qual = msr->flags.timing_qual;
 ...
 *
 * Revision 1.3  2014/03/04 19:22:15  dechavez
 * added support for MSFlags in msr_init() and msr_normalize_header()
 *
 * Revision 1.2  2014/03/04 19:17:33  dechavez
