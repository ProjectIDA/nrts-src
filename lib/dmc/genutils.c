#pragma ident "$Id: genutils.c,v 1.1 2014/07/10 20:30:42 dechavez Exp $"
/***************************************************************************
 * genutils.c
 *
 ...
 * ms_nomsampinterval:
 *
 * Calculate sample interval in nanoseconds from SEED sample rate factor and
 * multiplier as stored in the fixed section header of data records.
 * 
 * Returns the positive sample interval.
 ***************************************************************************/
uint64_t
ms_nomsampinterval (int factor, int multiplier)
{
uint64_t result;

#ifndef NANOSEC_PER_SEC
#define NANOSEC_PER_SEC 1000000000000
#endif /* !NANOSEC_PER_SEC */

    if (factor == 0 || multiplier == 0) return 0;

    if (factor > 0 && multiplier > 0) {
        result = NANOSEC_PER_SEC / factor * multiplier;
    } else if (factor > 0 && multiplier < 0) {
        result = -multiplier * NANOSEC_PER_SEC / factor;
    } else if (factor < 0 && multiplier > 0) {
        result = -factor * NANOSEC_PER_SEC / multiplier;
    } else { // (factor < 0 && multiplier < 0)
        result = -factor * NANOSEC_PER_SEC / -multiplier;
    }

    return result;

}  /* End of ms_nomsampinterval() */
 ...
 * ms_initflags:
 *
 * Initialize a MSFlag structure.  No return value.
 * 
 ***************************************************************************/
void
ms_initflags (MSFlags *flags)
{
  if ( flags == NULL ) return;
  memset(flags, 0, sizeof(MSFlags));
  flags->timing_qual = -1;
}  /* End of ms_initflags() */
 ...
 * ms_flagsmatch:
 *
 * Compare two MSFlag structures, return 1 if the fields are identical
 * and 0 if they are not.  Return -1 if either argument is NULL.
 * 
 ***************************************************************************/
flag
ms_flagsmatch (MSFlags *a, MSFlags *b)
{
  if ( a == NULL || b == NULL) return -1;
  return (memcmp(a, b, sizeof (MSFlags)) == 0) ? 1 : 0;
}  /* End of ms_flagsmatch() */
 ...
 *
 * Revision 1.4  2014/04/14 18:52:21  dechavez
 * added ms_nomsampinterval()
 *
 * Revision 1.3  2014/03/04 19:15:55  dechavez
 * added ms_initflags() and ms_flagsmatch()
 *
 * Revision 1.2  2014/03/04 19:14:27  dechavez
