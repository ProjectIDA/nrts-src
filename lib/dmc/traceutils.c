#pragma ident "$Id: traceutils.c,v 1.1 2014/07/10 20:30:43 dechavez Exp $"
/***************************************************************************
 * traceutils.c:
 *
 ...
 * dataquality byte is not 0 it must also match.  Similarily, if the
 * MSFlags argument is non-null, then it's contents must match.
 *
  ...
  *whence = 0;
  ...
  while ( mst )
      ...
      if ( channel[idx] != '\0' || mst->channel[idx] != '\0' )
	{
      ...
      /* Compare flags */
      if ( flags != NULL && !ms_flagsmatch(flags, &mst->flags) )
	{
	  mst = mst->next;
	  continue;
	}
		    ...
		    double timetol, double sampratetol )
{
  ...
  mst = mst_findadjacent (mstg, &whence, dq,
			  ...
			  msr->starttime, endtime, timetol, &msr->flags);
  ...
  if ( mst )
  ...
  else
    {
      ...
      memcpy(&mst->flags, &msr->flags, sizeof(MSFlags));
  ...
  while ( curtrace )
      ...
      while ( nexttrace )
	{
	  ...
	  if ( strcmp (searchtrace->network, curtrace->network) ||
	       ...
	       !ms_flagsmatch (&searchtrace->flags, &curtrace->flags) )
	    {
	      prevtrace = searchtrace;
	      continue;
	    }
 ...
 *
 * Revision 1.4  2014/03/04 20:25:40  dechavez
 * initialize MSFlags in mst_init()
 *
 * Revision 1.3  2014/03/04 19:49:48  dechavez
 * added support for MSFlags in mst_init(), mst_findadjacent(), mst_addmsrtogroup(), mst_groupheal(), mst_pack()
 *
 * Revision 1.2  2014/03/04 19:23:39  dechavez
