#pragma ident "$Id: util.c,v 1.17 2015/11/14 00:40:47 dechavez Exp $"
/*======================================================================
 *
...
/* Send a message consisting of a UINT16 payload */

BOOL iacpSendUINT16(IACP *iacp, UINT32 type, UINT16 value)
{
UINT16 buf;
IACP_FRAME frame;
    ...
    frame.payload.len  = sizeof(UINT16);
    frame.payload.data = (UINT8 *) &buf;
    utilPackUINT16(frame.payload.data, value);
...
char tstamp[TSLEN];
    ...
    fprintf(iacp->dbgfp, "%s peer=%s seqno=%u type=%u payload len=%u\n",
...
char tstamp[TSLEN];
    ...
    now = time(NULL);
    if (strftime(tstamp, TSLEN, TSFMT, localtime(&now))) fprintf(iacp->dbgfp, "%s: %s (%u bytes)\n", tstamp, prefix, frame->payload.len);
...
char tstamp[TSLEN];
    ...
    now = time(NULL);
    if (strftime(tstamp, TSLEN, TSFMT, localtime(&now))) fprintf(iacp->dbgfp, "%s: ", tstamp);
    fprintf(iacp->dbgfp, "%s peer=%s seqno=%u id=%u len=%u\n",
...
VOID iacpInitIncoming(IACP_MULTI_MSG *incoming)
{
    incoming->type = 0;
...
VOID iacpResetIncoming(IACP_MULTI_MSG *incoming)
{
 ...
 *
 * $Log: util.c,v $
 * Revision 1.17  2015/11/14 00:40:47  dechavez
 * changed changed fprintf format strings to calm Darwin compiles
 *
 * Revision 1.16  2012/06/27 15:26:57  dechavez
 * added iacpInitIncoming(), iacpResetIncoming()
 *
 * Revision 1.15  2010/08/27 18:50:00  dechavez
 * added iacpSendUINT16()
 *
