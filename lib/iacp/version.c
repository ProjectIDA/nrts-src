#pragma ident "$Id: version.c,v 1.34 2015/11/14 00:41:08 dechavez Exp $"
/*======================================================================
 * 
...
static VERSION version = {1, 10, 1};

/* iacp library release notes

1.10.1  11/13/2015
        accept.c: declare len as unsigned int instead of int
        connect.c: fixed long standing bug in setting peer ident
        handshake.c: changed sprintf format string to calm Darwin compiles
        string.c: changed sprintf format string to calm Darwin compiles
        util.c: changed changed fprintf format strings to calm Darwin compiles

1.10.0  06/27/2012
        iacp.h: added IACP_MULTI_MSG (was isi.h ISI_INCOMING)
        get.c: added iacpGetLogio()
        util.c: added iacpInitIncoming(), iacpResetIncoming()

1.9.0   10/12/2011
        connect.c: only retry connections when at_retry flag set in attributes

1.8.0   08/27/2010
        util.c: added iacpSendUINT16()
