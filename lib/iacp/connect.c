#pragma ident "$Id: connect.c,v 1.16 2015/11/14 00:38:45 dechavez Exp $"
/*======================================================================
 * 
...
/* must have encountered an error, either in connect or handshake */
        ...
        if (!retry || ((UINT32) ErrorLevel > iacpGetMaxError(iacp))) {
            return FALSE;
        } else {
            iacp->sd = utilCloseSocket(iacp->sd);
 ...
 *
 * $Log: connect.c,v $
 * Revision 1.16  2015/11/14 00:38:45  dechavez
 * fixed long standing bug in setting peer ident
 *
 * Revision 1.15  2011/10/12 17:26:10  dechavez
 * only retry connections when at_retry flag set in attributes (via iacpGetRetry)
 *
