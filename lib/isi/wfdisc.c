#pragma ident "$Id: wfdisc.c,v 1.11 2015/12/04 22:01:56 dechavez Exp $"
/*======================================================================
 *
...
{
LNKLST_NODE *crnt;
#define LOCAL_BUFLEN CSSIO_WFDISC_SIZE
...
IACP_FRAME frame;
char string[CSSIO_WFDISC_SIZE];
#define LOCALBUFLEN (CSSIO_WFDISC_SIZE * 2)
    ...
    do {
        ...
        switch (frame.payload.type) {
          ...
          case ISI_IACP_WFDISC:
            if (frame.payload.len == CSSIO_WFDISC_SIZE-1) {
                strlcpy(string, (const char *) frame.payload.data, CSSIO_WFDISC_SIZE);
                if (!listAppend(list, string, CSSIO_WFDISC_SIZE)) return FALSE;
                begin = time(NULL);
            }
            break;
 ...
 *
 * $Log: wfdisc.c,v $
 * Revision 1.11  2015/12/04 22:01:56  dechavez
 * explict cast of arguments and format strings to calm OS X compiles
 *
 * Revision 1.10  2011/03/17 17:35:48  dechavez
