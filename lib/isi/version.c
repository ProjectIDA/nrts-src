#pragma ident "$Id: version.c,v 1.83 2017/10/20 01:38:17 dauerbach Exp $"
/*======================================================================
 *
...
static VERSION version = {2, 18, 2};

/* isi library release notes

2.18.2 10/19/2017
       string.c: commented out dead code referencing libliss routines.

2.18.1 12/04/2015
       isi.h: added missing prototypes
       datreq.c: explict cast of arguments and format strings to calm OS X compiles
       fileio.c:   "
       pack.c:     "
       read.c:     "
       soh.c:      "
...
2.18.0 08/28/2014
       isi.h:     removed ISI_MSEED_EXTRA support
       mstrace.c: removed
       raw.c:     removed isiToMseed()
       utils.c:   removed isiAllocMseedExtras()

2.17.1 04/30/2014
       mstrace.c: fixed io_flag typo, removed unused code

2.17.0 04/14/2014
       mstrace.c: new file, introduces isiToMSTrace()

2.16.3 01/30/2014
       string.c: changed isiDatatypeString() to return f4/f8 for little endian floats

2.16.2 01/27/2014
       push.c: changed (internal function) SendFrame() to void and removed uneeded return

2.16.1 10/23/2012
       push.c: changed errno == ECONNRESET (comparison) to errno = ECONNRESET (assignment)

2.16.0 01/14/2012
       isi.h: added isiChnlocToChnLoc() prototype
       utils.c: added isiChnlocToChnLoc()

2.15.0 06/27/2012
       isi.h: replaced ISI_INCOMING with IACP_MULTI_MSG from iacp.h (it's exactly the same structure)
       datreq.c: isiResetIncoming() replaced with iacpResetIncoming()
       netutil.c: removed isiInitIncoming() and isiResetIncoming() (now part of libiacp)
       open.c: isiInitIncoming() replaced with iacpInitIncoming()
       utils.h: fixed isiAllocMseedExtras() bug when splitting long station names into 4+2 sta/net
...
2.14.0 11/07/2011
       isi.h: removed INCLUDE_MSEED conditionals
       mseed.c: created: isiRawToMiniSeed()
       raw.c: removed INCLUDE_MSEED conditionals
       string.c: removed INCLUDE_MSEED conditionals
       utils.c: removed INCLUDE_MSEED conditionals

2.13.0 11/02/2011 (initial support of MiniSEED data requests)
       isi.h: added UINT8 mseed array to ISI_RAW_HEADER (fixed length ISI_MSEED_HDR_LEN)
       ...
       pack.c: added support for packing and unpacking mseed() field of ISI_RAW_HEADER
       raw.c: added isiToMseed()
       string.c: use isiSeqnoString2() in isiRawHeaderString (removes the spaces in the seqno printout),
          also include MiniSEED header in string if MiniSEED present and INCLUDE_MSEED defined
       utils.c: added isiAllocMseedExtras() and conditional (INCLUDE_MSEED) support for ISI_TYPE_MSEED

2.12.2 10/31/2011 (getting ready for miniseed requests, but not there)
       raw.c: initialize mseed in isiInitRawHeader()
       string.c: added optional support for mt-unsafe NULL output buffers

2.12.1 10/29/2011 (getting ready for miniseed requests, but not there)
       isi.h: added MSEED types (packet type, format, tag)
       string.c: added miniseed support to isiFormatString()

2.12.0 10/12/2011
       push.c: added internal buffering into push client

2.11.0 08/04/2011
       isi.h: added ISI_PUSH and prototypes
       push.c: initial release

2.10.0 07/27/2011
       string.c: added isiSeqnoString2()

2.9.1  03/17/2011
       wfdisc.c: use the new CSSIO_, cssio_, prefixed names from libcssio 2.2.0

2.9.0  11/10/2010
       isi.h: added ISI_CONTENTS
       raw.c: ISI_CONTENTS support, initialize oldseqno in isiInitRawHeader()
       pack.c: added support for tagged contents (ISI_CONTENTS) in ISI_RAW_PACKET

2.8.1  09/10/2010
       ezio.c: added isiGetTcpBuflen()
       pack.c: added support for tagged oldseqno in ISI_RAW_PACKET

2.8.0  08/27/2010
       ezio.c: added isiReport(), isiFreeReport()
       pack.c: added isiPackReportEntry(), isiUnpackReportEntry()
       report.c: initial release

2.7.1  07/23/2010
       raw.c: fixed byte alignment problem with uncompressed input in isiDecompressRawPacket()
