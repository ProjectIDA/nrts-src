#pragma ident "$Id: pack.c,v 1.19 2015/12/04 22:01:56 dechavez Exp $"
/*======================================================================
 *
...
{
UINT8 *ptr;
static char *fid = "isiUnpackSeqno";
...
{
UINT8 *ptr;
static char *fid = "isiPackContents";

    ptr = start;

    *ptr++ = src->type;
    if (src->type == ISI_CONTENTS_TYPE_TS) {
        ptr += isiPackStreamName(ptr, &src->name);
        ptr += isiPackTstamp(ptr, &src->tofs);
        ptr += isiPackTstamp(ptr, &src->tols);
    }
...
{
UINT8 *ptr;
static char *fid = "isiUnpackContents";

    ptr = start;
    dest->type = *ptr++;
    if (dest->type == ISI_CONTENTS_TYPE_TS) {
        ptr += isiUnpackStreamName(ptr, &dest->name);
        ptr += isiUnpackTstamp(ptr, &dest->tofs);
        ptr += isiUnpackTstamp(ptr, &dest->tols);
    }
...
UINT8 *ptr, *lptr;
    ...
    ptr += sizeof(INT32);
    ptr += (length = utilPackBytes(ptr, (UINT8 *) src, ISI_SITELEN));
...
static int packTaggedOldSeqno(UINT8 *start, ISI_SEQNO *src)
{
INT32 length;
UINT8 *ptr, *lptr;
    ...
    lptr = (ptr += utilPackINT32(ptr, ISI_TAG_OLD_SEQNO));
    ptr += sizeof(INT32);
    ptr += (length = isiPackSeqno(ptr, src));
    utilPackINT32(lptr, length);
...
static int packTaggedContents(UINT8 *start, ISI_CONTENTS *src)
{
INT32 length;
UINT8 *ptr, *lptr;
    ...
    lptr = (ptr += utilPackINT32(ptr, ISI_TAG_CONTENTS));
    ptr += sizeof(INT32);
    ptr += (length = isiPackContents(ptr, src));
    utilPackINT32(lptr, length);

    return (int) (ptr - start);
}

static int packTaggedMseed(UINT8 *start, UINT8 *src)
{
INT32 length;
UINT8 *ptr, *lptr;
    ...
    lptr = (ptr += utilPackINT32(ptr, ISI_TAG_MSEEDHDR));
    ptr += sizeof(INT32);
    ptr += (length = utilPackUINT8Array(ptr, src, ISI_MSEED_HDR_LEN));
    utilPackINT32(lptr, length);
...
UINT8 *ptr;
    ...
    if (!isiIsUndefinedSeqno(&src->oldseqno)) ptr += packTaggedOldSeqno(ptr, &src->oldseqno);
    if (src->contents.type != ISI_CONTENTS_TYPE_UNKNOWN) ptr += packTaggedContents(ptr, &src->contents);
    if (src->mseed[0] != 0) ptr += packTaggedMseed(ptr, src->mseed);
...
/* set defaults for those fields that are not necessarily universal */

    isiInitRawHeader(&dest->hdr);

/* unpack header contents, ignoring any items we may not recognize */
    ...
    while ((mainPtr = GetNextField(mainPtr, &type, &ptr)) != NULL) {
        switch (type) {
          ...
          case ISI_TAG_OLD_SEQNO:
            isiUnpackSeqno(ptr, &dest->hdr.oldseqno);
            break;

          case ISI_TAG_CONTENTS:
            isiUnpackContents(ptr, &dest->hdr.contents);
            break;

          case ISI_TAG_MSEEDHDR:
            utilUnpackUINT8Array(ptr, dest->hdr.mseed, ISI_MSEED_HDR_LEN);
            break;
...
UINT32 len;
    ...
    len = (src->len <= ISI_MAX_REPORT_LEN) ? src->len : 0; // len==0 tells client we are broken
    ptr = start;

    ptr += utilPackUINT16(ptr, src->type);
    ptr += utilPackUINT32(ptr, len);
    ptr += utilPackBytes(ptr, src->payload, len);
...
UINT32 len;
    ...
    dest->len = (len <= ISI_MAX_REPORT_LEN) ? len : 0; // dest->len==0 tells application we are broken
    ptr += utilUnpackBytes(ptr, dest->payload, dest->len);
 ...
 *
 * $Log: pack.c,v $
 * Revision 1.19  2015/12/04 22:01:56  dechavez
 * explict cast of arguments and format strings to calm OS X compiles
 *
 * Revision 1.18  2011/11/03 17:38:05  dechavez
 * added support for packing and unpacking mseed() field of ISI_RAW_HEADER
 *
 * Revision 1.17  2010/11/10 21:11:26  dechavez
 * added support for tagged contents (ISI_CONTENTS) in ISI_RAW_PACKET
 *
 * Revision 1.16  2010/09/10 22:50:43  dechavez
 * added support for tagged oldseqno in ISI_RAW_PACKET
 *
 * Revision 1.15  2010/08/27 18:51:22  dechavez
 * added isiPackReportEntry(), isiUnpackReportEntry()
 *
