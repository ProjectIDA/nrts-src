#pragma ident "$Id: string.c,v 1.35 2017/10/20 01:35:42 dauerbach Exp $"
/*======================================================================
 *
...
LISS_PKT liss;
    ...
    sprintf(buf+strlen(buf), " %s", isiSeqnoString2(&hdr->seqno, tbuf));
    sprintf(buf+strlen(buf), " %s", isiDescString(&hdr->desc, tbuf));
    sprintf(buf+strlen(buf), " %5u", hdr->len.used);
    sprintf(buf+strlen(buf), " %5u", hdr->len.native);
    ...
    /* might want to use libmseed if this is pursued */
    /* if (hdr->mseed[0] != 0) { */
    /*     lissUnpackMiniSeed(&liss, hdr->mseed, 0); */
    /*     sprintf(buf+strlen(buf), " %s", lissMiniSeedHdrString(&liss.hdr, tbuf)); */
    /* } */
    return buf;
}
...
static char mt_unsafe[ISI_SEQ_NO_STRING_LEN+1];
    ...
    if (isiIsOldestSeqno(seqno)) {
        ...
        sprintf(buf, "%08x", seqno->signature);
        sprintf(buf+strlen(buf), " %08x", (UINT32) (seqno->counter >> 32));
        sprintf(buf+strlen(buf), " %08x", (UINT32) seqno->counter & 0x00000000ffffffff);
    }
...
static char *null = " -";
    ...
    if (desc->order == ISI_ORDER_BIGENDIAN) {
        ...
        switch (desc->type) {
          ...
          case ISI_TYPE_REAL32: return f4;
          case ISI_TYPE_REAL64: return f8;
        }
    }
 ...
 *
 * $Log: string.c,v $
 * Revision 1.35  2017/10/20 01:35:42  dauerbach
 * commented out dead code that calls libliss routines
 *
 * Revision 1.34  2015/12/04 22:01:56  dechavez
 * explict cast of arguments and format strings to calm OS X compiles
 *
 * Revision 1.33  2014/01/30 19:51:51  dechavez
 * changed isiDatatypeString() to return f4/f8 for little endian floats
 *
 * Revision 1.32  2011/11/07 17:33:44  dechavez
 * removed INCLUDE_MSEED conditionals
 *
 * Revision 1.31  2011/11/03 17:36:07  dechavez
 * use isiSeqnoString2() in isiRawHeaderString (removes the spaces in the seqno printout),
 * also include MiniSEED header in string if MiniSEED present and INCLUDE_MSEED defined
 *
 * Revision 1.30  2011/10/31 17:04:36  dechavez
 * added optional support for mt-unsafe NULL output buffers
 *
 * Revision 1.29  2011/10/28 21:47:42  dechavez
 * added miniseed support to isiFormatString()
 *
 * Revision 1.28  2011/07/27 23:36:28  dechavez
 * added isiSeqnoString2()
 *
