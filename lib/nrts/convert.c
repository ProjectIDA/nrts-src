#pragma ident "$Id: convert.c,v 1.13 2015/12/04 23:42:15 dechavez Exp $"
/*======================================================================
 *
...
char chnloc[IDA10_CNAMLEN+1];
    ...
    *ptr++ = 10; /* format 10 */
    *ptr++ = 5;  /* subformat 5 */
    ptr += utilPackBytes(ptr, (UINT8 *) fsdh->staid, IDA105_SNAME_LEN);
    ptr += utilPackBytes(ptr, (UINT8 *) fsdh->netid, IDA105_NNAME_LEN);
...
/* Get conversion workspace for this stream */
    ...
    if ((work = GetBuffer(dl->ida10, &pkt.hdr.fsdh)) == NULL) {
...
/* Sanity checks */
    ...
    if (!VerifySampleInterval(&work->fsdh, &pkt.hdr.fsdh)) {
        logioMsg(dl->lp, LOG_INFO, "%s: WARNING - sample interval change %s.%s.%s", fid, pkt.hdr.fsdh.staid, pkt.hdr.fsdh.chnid, pkt.hdr.fsdh.locid);
        StartNewPacket(work, &pkt.hdr.fsdh, pkt.hdr.fsdh.start);
    } else {
        incerr = pkt.hdr.fsdh.start - work->tons;
        if (fabs(incerr) > work->sint) {
            smperr = (int) (incerr / work->sint);
            logioMsg(dl->lp, LOG_INFO, "%s: WARNING - %d sample %s.%s.%s. time tag increment error", fid, smperr, pkt.hdr.fsdh.staid, pkt.hdr.fsdh.chnid, pkt.hdr.fsdh.locid);
            StartNewPacket(work, &pkt.hdr.fsdh, pkt.hdr.fsdh.start);
        } else {
            work->tons = pkt.hdr.fsdh.start; /* sync to current packet to avoid accumulating one sample drift errors */
        }
    }
...
/* Build IDA10 packet, flush to disk loop when full */
    ...
    for (i = 0; i < pkt.hdr.fsdh.nsamp; i++) {
        ...
        if (work->nsamp == FIXED_NSAMP) {
            ...
            StartNewPacket(work, &pkt.hdr.fsdh, work->tons);
        }
    }
}
 ...
 *
 * $Log: convert.c,v $
 * Revision 1.13  2015/12/04 23:42:15  dechavez
 * casts, format fixes and the like, all to calm OS X compiles
 *
 * Revision 1.12  2011/11/03 17:47:39  dechavez
