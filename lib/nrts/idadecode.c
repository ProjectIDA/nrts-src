#pragma ident "$Id: idadecode.c,v 1.22 2015/12/04 23:42:15 dechavez Exp $"
/*======================================================================
 *
...
#define ONE_HOUR ((INT32) 3600)

static struct nrts_sys *sys;
static int             flags;
static int             shift;
static UINT32   errtol;
...
/* We can modify the flag and tic tolerance many times */
    ...
    if (opt != (void *) 0) {
        errtol = *((UINT32 *) opt);
        util_log(1, "time tears of %ld tics or less are tolerated", (INT32) errtol);
    } else {
        errtol = 0;
    }
...
static int check_tstamp(
    ...
    INT32 threshold, char *chname, char *response,
    int loglevel
) {
 ...
 */
    ...
    if (prev != (IDA_DHDR *) NULL) {
        ...
        diff = (INT32) ((crnt_off - prev_off) * 1000.0);
 ...
 */
    ...
    if (station_system) {
        now = (INT32) time(NULL);
        diff = now > (INT32) crnt->end.tru ? now - (INT32) crnt->end.tru :
                                             (INT32) crnt->end.tru - now;
        if (diff > threshold) {
            ...
            sprintf(msgbuf+strlen(msgbuf), "diff = %d secs", diff);
            util_log(loglevel, msgbuf);
            return -7;
...
/* Reload header to reflect modified data buffer contents */
    ...
    if ((status = ida_dhead(ida, crnt, (UINT8 *) buffer)) < 0) {
...
UINT32 uldiff;
    ...
    if (expected > crnt->end.tru) {
        uldiff = (UINT32) ((expected > crnt->end.tru) * 1000.0);
    } else {
        uldiff = (UINT32) ((crnt->end.tru - expected) * 1000.0);
    }
...
/* Decode the record header */
    ...
    if (ida_rtype((UINT8 *) buffer, ida->rev.value) != IDA_DATA) {
...
/*  Fill in the output header for this packet */
    ...
    strlcpy(hd.wfdisc.instype, channel_table[index].instype, CSSIO_INSTYPE_LEN+1);
...
/*  Finally, apply Fels' shifts if neccesary  */
    ...
    if (
        ...
        ldata = (INT32 *) (buffer + chn->hdr.len);
 ...
 *
 * $Log: idadecode.c,v $
 * Revision 1.22  2015/12/04 23:42:15  dechavez
 * casts, format fixes and the like, all to calm OS X compiles
 *
 * Revision 1.21  2012/02/14 20:19:04  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32)
 *
 * Revision 1.20  2011/03/17 17:34:19  dechavez
 * use the new CSSIO_, cssio_, prefixed names from libcssio 2.2.0
 *
 * Revision 1.19  2011/02/15 23:14:03  dechavez
 * rename sys->reclen to sys->deflen
 *
