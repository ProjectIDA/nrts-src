#pragma ident "$Id: version.c,v 1.87 2016/07/20 21:48:28 dechavez Exp $"
/*======================================================================
 * 
...
static VERSION version = {4, 16, 7};

/* nrts library release notes

4.16.7  07/20/2016
        nrts.h: Changed NRTS_DEFHOME to /Users/nrts for MacOS builds

4.16.6  12/05/2015
        asldecode.c: casts, format fixes and the like, all to calm OS X compiles
        convert.c:    "
        fixwd.c:      "
        ida.c:        "
        idadecode.c:  "
...
4.16.5  04/30/2014
        ida10.c: added support for new IDA10_CLOCK_STATUS percent field

4.16.1  03/12/2014
        oldbwd.c: renamed flag variable to action, to avoid conflict with include/dmc/libmseed.h flag type

4.16.0  01/30/2014
        ida10.c: set byte order flags to big-endian instead of native
                 This was causing byte order confusion with 16-bit data on little endian
                 hosts.  It is not clear why 32-bit data did not exhibit the same problem.

4.15.1  05/11/2013
        rcnf.h: permit IDA10 packets to be of any length

4.15.0  01/18/2013
        isi/dl.h: changed IDA9 (and below) work space to IDA_DREC in NRTS_PKT
        ida.c: decode (swapping into host byte order) IDA9 data in NRTS_PKT

4.14.1  01/14/2013
        ida.c, ida10.c: isiChnlocToChnLoc(&out->isi.hdr.name)

4.13.1  07/31/2012
        nrts/dl.h: defined NRTS_DL_FLAGS_ASYNC_WRITE option flag
        dl.c: nrtsOpenDiskLoop() to propagates ISI_GLOB flag ISI_DL_FLAGS_ASYNC_WRITE
          to NRTS_DL NRTS_DL_FLAGS_ASYNC_WRITE, and nrtsWriteDiskLoop() suppresses O_SYNC
          when that bit is set.

4.13.0  02/15/2012
        asldecode.c, bwd.c, chksys.c, fixwd.c, ida.c, ida10.c, ida10decode.c,
        idadecode.c, mmap.c, oldbwd.c, rcnf.c, stawd.c, syswd.c, time.c, wrtdl.c:
          use portable integer types (eg, long to INT32, unsigned long to UINT32)

4.12.1  11/02/2011
        convert.c: adjust for fsdh and b1000 moving into LISS_MSEED_HDR hdr field

4.12.0  03/17/2011
        use the new CSSIO_, cssio_, prefixed names from libcssio 2.2.0

4.11.0  02/15/2011
        nrts/types.h:  rename sys->reclen to sys->deflen
        ida10decode.c: rename sys->reclen to sys->deflen
        idadecode.c:   rename sys->reclen to sys->deflen
        wrtdl.c:       rename sys->reclen to sys->deflen
        rcnf.c:        allow optional override of default packet length on a per-channel basis

4.10.1  05/10/2010
        ida10.c: check for illegal nsint in TimeTearTest()
