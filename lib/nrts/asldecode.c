#pragma ident "$Id: asldecode.c,v 1.10 2016/06/23 20:19:10 dechavez Exp $"
/*======================================================================
 *
...
/* Save all current calib params for this station */
    ...
    for (i = 0; i < nchan; i++) {
        ...
        if (channel_table[i].index >= 0) {
            ...
            channel_table[i].vang = -999.9;
            channel_table[i].hang = -999.9;
            strcpy(channel_table[i].instype, "-");
...
/* Decode and determine if desired stream. */
    ...
    if ((status = seed_minihdr(crnt, (UINT8 *) buffer)) < 0) {
...
/*  Fill in the output header for this packet */
    ...
    strlcpy(hd.wfdisc.instype, channel->instype, NRTS_INAMLEN);
 ...
 *
 * $Log: asldecode.c,v $
 * Revision 1.10  2016/06/23 20:19:10  dechavez
 * fixed strlncpy buffer size off by one bug
 *
 * Revision 1.9  2015/12/04 23:42:15  dechavez
 * casts, format fixes and the like, all to calm OS X compiles
 *
 * Revision 1.8  2012/02/14 20:19:04  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32)
 *
 * Revision 1.7  2011/03/17 17:34:18  dechavez
