#pragma ident "$Id: ida.c,v 1.32 2015/12/04 23:42:15 dechavez Exp $"
/*====================================================================
 *
...
static char *hdrstring(IDA_DHDR *dhdr)
{
static char buf[1024];

    sprintf(buf, " %2d",  dhdr->wrdsiz);
    sprintf(buf+strlen(buf), " %4d",  dhdr->nsamp);
    sprintf(buf+strlen(buf), " %7.3f",dhdr->sint);
    sprintf(buf+strlen(buf), " %s",   sys_timstr(dhdr->beg.sys, dhdr->beg.mult));
    sprintf(buf+strlen(buf), "  %d",  dhdr->beg.qual);
    sprintf(buf+strlen(buf), " %s",   tru_timstr(&dhdr->beg));
    sprintf(buf+strlen(buf), " %s",   sys_timstr(dhdr->end.sys, dhdr->end.mult));
    sprintf(buf+strlen(buf), "  %d",  dhdr->end.qual);
    sprintf(buf+strlen(buf), " %s",   tru_timstr(&dhdr->end));
    if (dhdr->extra.valid) sprintf(buf+strlen(buf), " 0x%08x", dhdr->extra.seqno);
    return buf;
}
...
char tmp1[64], tmp2[64];
struct {INT32 beg; INT32 end;} diff;
 ...
 */
    ...
    /* If offsets match then we can be certain this packet is OK */
        ...
        diff.beg = (INT32) ((offset.crnt - offset.prev) * 1000.0); /* diff in msec */

        offset.prev = ida_ext1hzoff(&prev->end);
        offset.crnt = ida_ext1hzoff(&crnt->end);
        diff.end = (INT32) ((offset.crnt - offset.prev) * 1000.0); /* diff in msec */
...
char tmp1[64], tmp2[64];
    ...
    now = (INT32) time(NULL);
    diff = now > (INT32) crnt->end.tru ? now - (INT32) crnt->end.tru : (INT32) crnt->end.tru - now;
    if (diff > (INT32) dl->tol.clock) {
...
static char *IDA9 = IDA_REV9_TAG_0;
    ...
    if (memcmp(&raw[REV9_TAG_OFFSET], Ida9, strlen(Ida9)) == 0) { /* already Ida9 */
...
/* Set up pointers to raw packet */
    ...
    drec = &out->work.buf.ida;
    out->work.len = sizeof(IDA_DREC);

    out->orig.order = NATIVE_BYTE_ORDER; /* since that is what ida_drec does */
...
/* We are only interested in waveform data */
    ...
    if (ida_rtype(in, dl->ida->rev.value) != IDA_DATA) {
...
/* Decode the packet */
    ...
    if ((error = ida_drec(dl->ida, drec, in, FALSE, TRUE)) != 0) {
        logioMsg(dl->lp, LOG_ERR, "%s: ida_dhead error %d", fid, error);
        return FALSE;
    }
    out->isi.data = drec->data;
...
/* Convert to generic time series format */
    ...
    if (stream != NULL) {
        ...
        idaBuildStreamName(dl->ida, &drec->head, &out->isi.hdr.name);
    }
    isiStreamNameString(&out->isi.hdr.name, out->ident);
    isiChnlocToChnLoc(&out->isi.hdr.name);
    isiSintToSrate((REAL64) drec->head.sint, &out->isi.hdr.srate);

    out->isi.hdr.desc.order = ISI_ORDER_BIGENDIAN;
    if (drec->head.form == IGPP_COMP) {
        out->isi.hdr.desc.comp = ISI_COMP_IDA;
    } else {
        out->isi.hdr.desc.comp = ISI_COMP_NONE;
    }
    if (drec->head.wrdsiz == sizeof(INT32)) {
        out->isi.hdr.desc.type = ISI_TYPE_INT32;
        out->isi.hdr.desc.size = sizeof(INT32);
    } else if (drec->head.wrdsiz == sizeof(INT16)) {
        out->isi.hdr.desc.type = ISI_TYPE_INT16;
        out->isi.hdr.desc.size = sizeof(INT32);
    } else {
        logioMsg(dl->lp, LOG_ERR, "%s: unsupported sample size (%d)", fid, drec->head.wrdsiz);
        return FALSE;
    }

    out->isi.hdr.tofs.value  = drec->head.beg.tru;
    out->isi.hdr.tofs.status = drec->head.beg.qual;

    out->isi.hdr.tols.value  = drec->head.end.tru;
    out->isi.hdr.tols.status = drec->head.end.qual;
    out->isi.hdr.nsamp = (UINT32) drec->head.nsamp;

    out->isi.hdr.nbytes = drec->head.nbytes;

#ifdef DEBUG
    logioMsg(dl->lp, dl->debug->pkt, "%s: %s", fid, isiGenericTsHdrString(&out->isi.hdr, tmp));
#endif /* DEBUG */

    out->extra = drec->head.extra;
 ...
 *
 * $Log: ida.c,v $
 * Revision 1.32  2015/12/04 23:42:15  dechavez
 * casts, format fixes and the like, all to calm OS X compiles
 *
 * Revision 1.31  2013/01/18 23:36:59  dechavez
 * decode (swapping into host byte order) IDA9 data in NRTS_PKT
 *
 * Revision 1.30  2013/01/14 22:52:58  dechavez
 * fixed missing ; typo
 *
 * Revision 1.29  2013/01/14 22:42:51  dechavez
 * fully populate the ISI_STREAM_NAME structure (chn, and loc fields, specificially)
 *
 * Revision 1.28  2012/02/14 20:19:04  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32)
 *
 * Revision 1.27  2011/10/12 15:48:22  dechavez
