#pragma ident "$Id: ida10.c,v 1.30 2014/04/30 15:56:33 dechavez Exp $"
/*======================================================================
 *
...
char msgbuf[1024];
    ...
    if (crnt->nsint == 0) {
        logioMsg(dl->lp, LOG_WARN, "*** illegal sample interval (factor=%hd, multiplier=%hd)", crnt->dl.srate.factor, crnt->dl.srate.multiplier);
        return FALSE;
    }
...
char tmp1[64], tmp2[64];
    ...
    now = (INT32) time(NULL);
    diff = now > (INT32) crnt->tols ? now - (INT32) crnt->tols : (INT32) crnt->tols - now;
...
/* Station systems need to be a bit more strict than down stream systems */
    ...
    /* Reject obviously corrupt packets immediately */
        ...
        if (crnt->nsint == 0) return RejectPacket(dl, crnt, "BadSampleInterval");
...
UINT16 result;
    ...
    if (status->percent > 0) {
        result = (status->percent << 8) | ISI_TSTAMP_STATUS_HAVE_QUAL;
    } else {
        result = 0;
    }
...
static char *fid = "nrtsIda10Decoder";
    ...
    out->orig.order = BIG_ENDIAN_BYTE_ORDER; /* all IDA10 data are big-endian */
    ...
    out->isi.hdr.desc.order = BIG_ENDIAN_BYTE_ORDER; /* all IDA10 data are big-endian */
    ...
    out->isi.hdr.tols.status  = BuildIsiTstampStatus(&ts->hdr.cmn.ttag.end.status);
 ...
 *
 * $Log: ida10.c,v $
 * Revision 1.30  2014/04/30 15:56:33  dechavez
 * added support for new IDA10_CLOCK_STATUS percent field
 *
 * Revision 1.29  2014/01/30 19:31:14  dechavez
 * set byte order flags to big-endian instead of native
 *
 * Revision 1.28  2013/01/14 22:42:51  dechavez
 * fully populate the ISI_STREAM_NAME structure (chn, and loc fields, specificially)
 *
 * Revision 1.27  2012/02/14 20:19:04  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32)
 *
 * Revision 1.26  2010/05/10 16:22:00  dechavez
 * check for illegal nsint in TimeTearTest()
 *
