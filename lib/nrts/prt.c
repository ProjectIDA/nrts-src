#pragma ident "$Id: prt.c,v 1.6 2015/12/04 23:42:15 dechavez Exp $"
/*======================================================================
 *
...
static char *fid = "nrts_prtsta";
    ...
    if (what & NRTS_CONFIG) {
        ...
        for (i = 0; i < sta->nchn; i++) {
            ...
            sprintf(buffer+strlen(buffer), " %9ld",  chn->hdr.nrec);
            sprintf(buffer+strlen(buffer), " %9ld",  chn->hdr.hide);
            sprintf(buffer+strlen(buffer), " %9ld",  chn->hdr.nrec - chn->hdr.hide);
            sprintf(buffer+strlen(buffer), " %5d",   chn->hdr.len);
            sprintf(buffer+strlen(buffer), " %5d",   chn->dat.len);
            sprintf(buffer+strlen(buffer), " %9lld", chn->hdr.off);
            sprintf(buffer+strlen(buffer), " %9lld", chn->dat.off);
            bufprt(buffer);
        }
    }
    ...
    if (what & NRTS_INDICES) {
        ...
        for (i = 0; i < sta->nchn; i++) {
            ...
            bufset(buffer);
            sprintf(buffer+strlen(buffer), "%7s",    chn->name);
            sprintf(buffer+strlen(buffer), " %6ld",  chn->hdr.oldest);
            sprintf(buffer+strlen(buffer), " %6ld",  chn->hdr.yngest);
            sprintf(buffer+strlen(buffer), " %6ld",  chn->hdr.lend);
            sprintf(buffer+strlen(buffer), " %8s",   nrts_status(chn->status));
            sprintf(buffer+strlen(buffer), " %7ld",  chn->nread);
    ...
    if (what & NRTS_TIMES) {
        ...
        for (i = 0; i < sta->nchn; i++) {
            ...
            sprintf(buffer+strlen(buffer), " %4ld",   chn->nseg);
            sprintf(buffer+strlen(buffer), " %7ld",   chn->nread);
            sprintf(buffer+strlen(buffer), " %7.3f", chn->sint);
            lastwrite = time(NULL) - chn->tread;
            sprintf(buffer+strlen(buffer), " %s",  nrts_latency(lastwrite));
...
static char *fid = "nrts_prtsys";
    ...
    if (what & NRTS_SYSINFO) {
        bufset(buffer);
        sprintf(buffer+strlen(buffer), "host order = 0x%08lx ", sys->order);
 ...
 *
 * $Log: prt.c,v $
 * Revision 1.6  2015/12/04 23:42:15  dechavez
