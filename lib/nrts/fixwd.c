#pragma ident "$Id: fixwd.c,v 1.12 2015/12/04 23:42:15 dechavez Exp $"
/*======================================================================
 *
...
static int store_gap(INT32 index)
{
static struct gap_list *null = (struct gap_list *) NULL;
...
static INT32 gap_test(struct nrts_header *left, struct nrts_header *rite)
{
struct {
    double time;
    INT32   samp;
...
INT32 gap;
    ...
    span.samp = (INT32) (span.time * left->wfdisc.smprate);
...
static int process(INT32 a, INT32 b)
{
INT32 midpoint, gap;
int status;
static int level = 0;
...
static int update(INT32 a, INT32 b)
{
int done, yr, da, hr, mn, sc, ms;
INT32 begrec, endrec;
struct gap_list *crnt;
struct nrts_header beg, end, *ptr;
static int len = sizeof(struct cssio_wfdisc);
static struct gap_list *null = (struct gap_list *) NULL;
 ...
 *
 * $Log: fixwd.c,v $
 * Revision 1.12  2015/12/04 23:42:15  dechavez
 * casts, format fixes and the like, all to calm OS X compiles
 *
 * Revision 1.11  2012/02/14 20:19:04  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32)
 *
 * Revision 1.10  2011/03/17 17:34:18  dechavez
