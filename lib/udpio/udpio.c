...
#ifdef notdef
static void LogStats(char *which, LOGIO *lp, int level, UDPIO_STATS *stats, char *prefix, char *ident, UINT64 now)
{
#define TMPBUFLEN 256
char interval[TMPBUFLEN];

    utilTimeString(now - stats->tstamp, 108, interval, TMPBUFLEN);
    logio(lp, level, "%s %s %s %s len=%d min=%d total=%llu packets=%llu",
        prefix, ident, which, interval, stats->len.min, stats->len.max, stats->total.bytes, stats->total.pkts
    );
}

void udpioLogStats(int level, UDPIO *up, char *prefix)
{
UINT64 now;
static char *empty = "";
UDPIO_STATS xmit, recv;

    if (up == NULL) return;
    if (prefix == NULL) prefix = empty;
    now = utilTimeStamp();
    udpioGetStats(up, &xmit, &recv);

    LogStats("xmit", up->lp, level, &xmit, prefix, up->peer.ident, now);
    LogStats("recv", up->lp, level, &recv, prefix, up->peer.ident, now);
}
#endif /* notdef */
...
WSADATA lpWSAData;
    ...
    if (bind(up->sd, (struct sockaddr *) &addr, sizeof(addr)) != 0) {
        logioMsg(up->lp, LOG_ERR, "%s: bind (port %d): %s", fid, port, strerror(errno));
        return FALSE;
    }
...
static char *fid = "udpioRecv";
    ...
    while (got == 0) {
        error = select(FD_SETSIZE, &fds, NULL, NULL, &timeout);
        if (error > 0) {
 ...
 *
 * Revision 1.15  2016/07/20 23:19:13  dechavez
 * added a missing errno assignment to one of the input sanity checks in udpioRecv()
 *
 * Revision 1.14  2015/12/04 22:42:29  dechavez
 * use FD_SETSIZE instead of gettablesize() for select "width" arg (for OS X portability)
 *
 * Revision 1.13  2014/08/11 18:20:03  dechavez
 * use socklen_t instead of int for socket length
 *
 * Revision 1.12  2010/12/06 17:25:02  dechavez
 * added some ifdef'd out code for possible future logging use
 *
 * Revision 1.11  2010/04/02 18:12:40  dechavez
 * include port number in bind() failure log message
 *
