#pragma ident "$Id: modbus-tcp.c,v 1.3 2016/08/26 20:10:31 dechavez Exp $"
/*
...
static int SetHostAddr(struct sockaddr_in *in_addr, char *host, int port)
{
{
unsigned long addr;
struct hostent *hp;
...
static int _modbus_tcp_connect(modbus_t *ctx)
{
    ...
    if (SetHostAddr(&addr, ctx_tcp->ip, ctx_tcp->port) != 0) {
        return -1;
    }
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    ret_size = modbus_strlcpy(ctx_tcp->ip, ip, dest_size);
    if (ret_size == 0) {
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    ret_size = modbus_strlcpy(ctx_tcp_pi->node, node, dest_size);
    if (ret_size == 0) {
    ...
    ret_size = modbus_strlcpy(ctx_tcp_pi->service, service, dest_size);
    if (ret_size == 0) {
