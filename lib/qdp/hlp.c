#pragma ident "$Id: hlp.c,v 1.12 2017/01/20 17:14:15 dechavez Exp $"
/*======================================================================
 *
...
QDP_HLP_RULES *rules;
    ...
    rules = &lcq->par.rules;
    hlp = lc->hlp;
    if (!rules->valid || hlp == NULL || hlp->nbyte == 0) return;
...
QDP_HLP *hlp;
QDP_HLP_RULES *rules;
BOOL InvokeOneSecOverlapCheck;
static char *fid = "qdpBuildHLP:BuildHLP:Uncompressed";

    rules = &lcq->par.rules;
    hlp = lc->hlp;

/* Check for time tears between these data and those previously appended to the
 * packet under construction.  Overlaps of exactly 1-second are expected when
 * reconnecting, so we check for that and toss the incoming data when it happens.
 * All other time tears are dealt with by flushing the partial packet early and
 * starting a new one.
 *
 * In the case of a leap year one would not want to toss the 1-sec overlaps
 * so that behavior can be overridden by setting the QDP_HLP_RULE_FLAG_NOHLP_1SECHK
 * flag bit.
 */

    InvokeOneSecOverlapCheck = (rules->flags & QDP_HLP_RULE_FLAG_NOHLP_1SECHK) ? FALSE : TRUE;
  
 /* On a side note, 5 minutes ago Donald J Trump was sworn in as
  * President of the United States. A little bit of me just died.
  */

    tofs = qdpDelayCorrectedTime(&lcq->mn232, hlp->delay);
    if (hlp->nsamp != 0) {
        errsec = TimeIncrError(hlp->tols, tofs, hlp->nsint);
        if (InvokeOneSecOverlapCheck) {
            if (errsec == -((INT64) NANOSEC_PER_SEC)) {
                return TRUE;
            } else if (hlp->nsint == 10 * NANOSEC_PER_SEC && errsec == -10 * ((INT64) NANOSEC_PER_SEC)) {
                return TRUE; /* special handling of the 1-sec overlap for the 10 sec streams */
            }
        }
        if (errsec != 0) {
...
/* Append the sample(s) to the packet */
    ...
    for (ptr = &hlp->data[hlp->nbyte], i = 0; i < lc->nsamp; i++) {
        ...
        if (lcq->cal_in_progress) hlp->cal_in_progress = TRUE;
...
static char *fid = "qdpInitHLP";
    ...
    memcpy(hlp->src, lc->src, QDP_LCQSRC_LEN);
 ...
 *
 * $Log: hlp.c,v $
 * Revision 1.12  2017/01/20 17:14:15  dechavez
 * added QDP_HLP_RULE_FLAG_NOHLP_1SECHK to BuildUncompressed()
 *
 * Revision 1.11  2013/02/07 18:18:41  dechavez
 * toss the 1-sec overlaps that follow reconnects (this addresses the short
 * packet followed by overlap mess that comes with bad comm links)
 *
 * Revision 1.10  2012/08/07 17:42:36  dechavez
 * fixed qdpDestroyHLP() declaration (removed extraneous *)
 *
 * Revision 1.9  2010/03/31 20:34:23  dechavez
 * added qdpFlushLCQ()
 *
 * Revision 1.8  2010/03/12 00:28:02  dechavez
 * added LCQ src bytes to QDP_HLP
 *
 * Revision 1.7  2009/07/09 18:17:26  dechavez
 * added cal_in_progress flag
 *
