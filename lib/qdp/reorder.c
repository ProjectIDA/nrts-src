#pragma ident "$Id: reorder.c,v 1.8 2016/07/20 17:20:23 dechavez Exp $"
/*======================================================================
 * 
...
{
QDP_PKT *old, *new;
static char *fid = "ActionDATA:DuplicateData";
    ...
    if (old->len != new->len || memcmp(old->raw, new->raw, old->len) != 0) {
        qdpError(qp, "FATAL ERROR: %s: slot %hu is already full!", fid, index);
        LogDup(qp, old, new);
        ActionTLD(qp, event);
        return TRUE;
    }
 ...
 *
 * $Log: reorder.c,v $
 * Revision 1.8  2016/07/20 17:20:23  dechavez
 * ActionDATA:DuplicateData() sanity check fails with ActionTLD() instead of no longer used ActionFAIL()
 *
