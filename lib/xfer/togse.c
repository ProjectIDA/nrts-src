#pragma ident "$Id: togse.c,v 1.5 2015/12/04 22:31:17 dechavez Exp $"
/*======================================================================
 *
...
struct chn_hist {
    ...
    double prev_tofs;
    struct {
        INT32 *raw; /* mmap'd to raw disk file */
...
/*  Build the header string  */
    ...
    sprintf(gsebuf+strlen(gsebuf), " %8d",    hdr->nsamp);
...
/* mmap raw data file to array  */
    ...
    chn->data.raw = (INT32 *) mmap(addr, len, prot, flag, fd, off);

    if (chn->data.raw == (INT32 *) -1) {
...
static char *fid = "Xfer_ToGSE:Rwrtdat";
    ...
    if (fwrite(
        packet->data, sizeof(INT32), packet->nsamp, chn->raw
 ...
 *
 * $Log: togse.c,v $
 * Revision 1.5  2015/12/04 22:31:17  dechavez
