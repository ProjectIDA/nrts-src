#pragma ident "$Id: site.c,v 1.15 2015/12/04 22:31:17 dechavez Exp $"
/*======================================================================
 *
...
INT32 nsamp;
static union {
    INT32 l[IDA_MAXDLEN/4];   /* uncompressed long   */
    INT16 s[IDA_MAXDLEN/2];   /* uncompressed shorts */
} data;
...
/* Non-data and corrupt data records are sent uncompressed */
    ...
    if (
        ida_rtype((UINT8 *)input, ida->rev.value) != IDA_DATA ||
        ida_dhead(ida, &dhead, (UINT8 *) input) != 0 ||
...
/* Do the compression */
    ...
    switch (*comp) {
      ...
      case XFER_CMPIGPP:
        ...
        if (dhead.wrdsiz == 4) {
            memcpy((char *) data.l, (UINT8 *) input + hlen, dlen);
            if (dhead.order != util_order()) util_lswap(data.l, nsamp);
            cbyte = util_lcomp((UINT8 *) cdata, data.l, nsamp);
        } else {
            memcpy((char *) data.s, (UINT8 *) input + hlen, dlen);
            if (dhead.order != util_order()) util_sswap(data.s, nsamp);
            cbyte = util_scomp((UINT8 *) cdata, data.s, nsamp);
        }
...
static char *idaN_decompress(IDA *ida, int *comp, INT32 *nbyte, char *input)
{
IDA_DHDR dhead;
...
/* Do the decompression, restoring original byte order if required */
    ...
    switch (*comp) {
      ...
      case XFER_CMPIGPP:
        ...
        if (dhead.wrdsiz == 4) {
            util_ldcmp(data.l, (UINT8 *) input + hlen, nsamp);
            if (dhead.order != util_order()) util_lswap(data.l, nsamp);
            memcpy(output + hlen, (char *) data.l, dlen);
        } else {
            util_sdcmp(data.s, (UINT8 *) input + hlen, nsamp);
            if (dhead.order != util_order()) util_sswap(data.s, nsamp);
...
static char *ida10_decompress(int *comp, INT32 *nbyte, char *input)
{
    return (char *) NULL;
}
    ...
    switch (*comp) {
      ...
      case XFER_CMPIGPP:
        ...
        if (ts.hdr.datatype == IDA10_DATA_INT32) {
            cbyte = util_lcomp((UINT8 *) cdata, (INT32 *) ts.data.int32, ts.hdr.nsamp);
...
/* Decompress the data portion, restore to NBO */
    ...
    switch (*comp) {
      ...
      case XFER_CMPIGPP:
        ...
        lptr = (INT32 *) (output + IDA10_TSHEADLEN);
        util_ldcmp(lptr, (UINT8 *) input + IDA10_TSHEADLEN, tshdr.nsamp);
...
{
    if (ida->rev.value != 10) {
        return idaN_compress(ida, comp, (char *) input, (char *) output);
    } else {
        return ida10_compress(comp, (char *) input, (char *) output, datlen);
    }
}
...
{
    if (ida->rev.value != 10) {
        return (UINT8 *) idaN_decompress(ida, comp, nbyte, (char *) input);
    } else {
        return (UINT8 *) ida10_decompress(comp, nbyte, (char *) input);
    }
}
 ...
 *
 * $Log: site.c,v $
 * Revision 1.15  2015/12/04 22:31:17  dechavez
 * casts, format fixes and benign bug fixes to calm OS X compiles
 *
 * Revision 1.14  2012/02/14 20:22:33  dechavez
