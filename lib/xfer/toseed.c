#pragma ident "$Id: toseed.c,v 1.6 2015/12/04 22:31:17 dechavez Exp $"
/*======================================================================
 * 
...
static char *tmplate = "./Xfer_ToSEED:newfile.XXXXXX";
    ...
    if ((new->info.name = mktemp(tmplate)) == (char *) NULL) {
        util_log(1, "%s: mktemp: %s: %s", fid, tmplate, syserrmsg(errno));
        return (struct file_info *) NULL;
    }
...
static char *fid = "Xfer_ToSEED:Rwrtdat";
    ...
    if (fwrite(
        packet->data, sizeof(INT32), packet->nsamp, info->fp
...
off_t off;
size_t len;
INT32 *input, remain, nc, used, seqno, reclen, buflen;
...
/* Check input, and allocate work space */
    ...
    reclen = (INT32) pow(2.0, (double) explen);
...
/* Demultiplex raw data to disk */
    ...
    while (Xfer_RdPacket(ifp, &packet) == 0) {
        ...
        } else if ((status = Rwrtdat(info, &packet, buffer, dupflag)) != 0) {
...
/* Process through the accumulated list of demux files */
    ...
    /* mmap raw data file to input array */
        ...
        input = (INT32 *) mmap(addr, len, prot, flag, fd, off);
    ...
    /* Convert to Steim 1 compressed mini-SEED */
        ...
        while (remain > 0) {
            ...
            nc = seed_mini(input,remain,&info[i].fsdh,(UINT8 *)buffer,explen,&used);
 ...
 *
 * $Log: toseed.c,v $
 * Revision 1.6  2015/12/04 22:31:17  dechavez
 * casts, format fixes and benign bug fixes to calm OS X compiles
 *
 * Revision 1.5  2012/02/14 20:22:33  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32)
 *
 * Revision 1.4  2009/07/27 17:45:31  dechavez
 * use mktemp() instead of tempnam()
 *
