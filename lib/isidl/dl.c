...
static char *fid = "LockDiskLoop";
    ...
    if (dl->lock.fp == NULL) {
        logioMsg(dl->lp, dl->glob->debug.lock, "%s: %s: ignored (lock fp is NULL)", pfid, fid);
        return;
    }
    ...
    if (dl->lock.perm == ISI_RDWR) {
        ...
        if (!utilWriteLockWait(dl->lock.fp)) {
            logioMsg(dl->lp, LOG_ERR, "%s: %s: utilWriteLockWait: %s: %s", pfid, fid, dl->sys->site, strerror(errno));
            return;
        }
    } else {
        //logioMsg(dl->lp, dl->glob->debug.lock, "%s: %s: utilReadLockWait", pfid, fid);
        if (!utilReadLockWait(dl->lock.fp)) {
            logioMsg(dl->lp, LOG_ERR, "%s: %s: utilReadLockWait: %s: %s", pfid, fid, dl->sys->site, strerror(errno));
            return;
        }
    }
}
...
static char *fid = "CreateHandle";
    ...
    handle->slink.enabled        = FALSE;
    handle->slink.first.acquired = TRUE;
    handle->slink.first.sent     = TRUE;
    handle->slink.mseed          = NULL;
    handle->slink.dlcp           = NULL;
...
static void InitNewSys(ISI_DL_SYS *sys, char *site, UINT32 numpkt, UINT32 nhide, UINT32 maxlen)
{
    ...
    sys->nhide  = nhide;
    sys->maxlen = maxlen;
    sys->hdrlen = sizeof(ISI_RAW_HEADER);
    sys->index.active = 0;
...
/* make sure we aren't incompatible with existing disk loops */

    if (dl->sys->hdrlen > sizeof(ISI_RAW_HEADER)) {
        logioMsg(lp, LOG_ERR, "%s: *** ERROR *** '%s' disk loop is incompatible with this release *** ERROR ***", fid, site);
        return DeleteHandleAndFail(dl);
    }
 ...
 */
    ...
    wflags = (dl->flags & ISI_DL_FLAGS_ASYNC_WRITE) ? O_WRONLY | O_BINARY : O_WRONLY | O_BINARY | O_SYNC;

    if ((dl->fd.hdr = open(dl->path.hdr, wflags)) < 0) {
        logioMsg(dl->lp, LOG_ERR, "%s: open: %s: %s:", fid, dl->path.hdr, strerror(errno));
        return DeleteHandleAndFail(dl);
    }

    if ((dl->fd.raw = open(dl->path.raw, wflags)) < 0) {
        logioMsg(dl->lp, LOG_ERR, "%s: open: %s: %s:", fid, dl->path.raw, strerror(errno));
        return DeleteHandleAndFail(dl);
    }

    isidlInitOptions(&dl->options, options);
...
static char *fid = "isidlCloseDiskLoop";
    ...
    if (dl == NULL) return;
    LockDiskLoop(fid, dl);
        if (dl->slink.enabled) isidlFeedSeedLink(&dl->slink, NULL); /* flush SeedLink */
...
off_t offset;
    ...
    offset = dl->sys->hdrlen * (off_t) index;
    ...
    if (write(dl->fd.hdr, &raw->hdr, dl->sys->hdrlen) != dl->sys->hdrlen) {
...
/* Add the host timestamp now, if requested.  We have to do it here
 * BEFORE doing the NRTS update because it will be needed if the
 * data header time stamps need to get patched.
 */

    if (dl->options.flags & ISI_OPTION_INSERT_32BIT_TSTAMP && dl->options.offset.tstamp != -1) {
        utilPackUINT32(&raw->payload[dl->options.offset.tstamp], (UINT32) time(NULL) - SAN_EPOCH_TO_1970_EPOCH);
    }
 ...
 */
    ...
    if (dl->nrts != NULL && NRTSCompatiblePacket(dl->nrts, raw) && raw->hdr.status == ISI_RAW_STATUS_OK) {
        ...
        /* Duplicates and overlaps may be cause for early return (permanently disappeared) */
          ...
          case NRTS_DUPLICATE:
            if (dl->options.flags & ISI_OPTION_REJECT_DUPLICATES) {
                tofsString = isiTstampString(&pkt->isi.hdr.tofs, tmp);
          ...
          case NRTS_COMPLETE_OVERLAP:
            if (dl->options.flags & ISI_OPTION_REJECT_COMPLETE_OVERLAPS) {
                tofsString = isiTstampString(&pkt->isi.hdr.tofs, tmp);
...
/* Write (possibly patched) raw packet */
    ...
    LockDiskLoop(fid, dl);
        ...
        isidlSysSetState(dl, ISI_ACTIVE, TRUE);
            ...
            if (dl->options.flags & ISI_OPTION_GENERATE_SEQNO) {
                raw->hdr.seqno = isidlSysIncrSeqno(dl, TRUE);
            } else {
                isidlSysSetSeqno(dl, &raw->hdr.seqno, TRUE);
            }

            if (dl->options.flags & ISI_OPTION_INSERT_32BIT_SEQNO && dl->options.offset.seqno != -1) {
                utilPackUINT32(&raw->payload[dl->options.offset.seqno], (UINT32) raw->hdr.seqno.counter);
            }
...
/* Tee to disk, if enabled */

    if (!dl->tee.disabled) isidlWriteToTeeFile(dl, raw);

/* Tee to SeedLink thread, if enabled */

    if (dl->slink.enabled) isidlFeedSeedLink(&dl->slink, raw);
...
UINT32 allocated;
    ...
    if (read(fd, dest, dl->sys->hdrlen) != dl->sys->hdrlen) {
 ...
 *
 * Revision 1.46  2015/12/04 22:14:06  dechavez
 * fixed sloppy block comment delimiter
 *
 * Revision 1.45  2014/08/28 21:21:31  dechavez
 * support move of options from its own entity to a member of the ISI_DL handle
 *
 * Revision 1.44  2014/08/11 18:29:59  dechavez
 * calls to isidlFeedSeedLink() changed to support new inteface in modified slink.c
 *
 * Revision 1.43  2014/04/30 15:53:45  dechavez
 * mods to support change of slink.mseed from MSEED_HANDLE to void pointer
 *
 * Revision 1.42  2013/03/08 18:31:10  dechavez
 * isidlCloseDiskLoop() flushes partial MiniSEED packets
 *
 * Revision 1.41  2013/01/14 22:11:07  dechavez
 * tee to SeedLink option enabled, BUD option now ignored (will prune later)
 *
 * Revision 1.40  2012/07/30 16:57:43  dechavez
 * isidlOpenDiskLoop() to NOT use O_SYNC for ISI writes when ISI_DL_FLAGS_ASYNC_WRITE selected
 *
 * Revision 1.39  2011/12/15 21:44:46  dechavez
 * insert host time stamp in IDA packet header before updating NRTS
 * (that allows it to be there for use by the NRTS sanity checks)
 *
 * Revision 1.38  2011/11/07 17:28:20  dechavez
 * if ISI_DL_OPTION mseed is set, create equivalent MiniSEED header before writing
 * packets to ISI disk loop, and tee those files to BUD if ISI_DL_BUD is set
 *
 * Revision 1.37  2011/10/12 17:19:53  dechavez
 * use ISI_DL_OPTIONS to insert 32-bit seqno and timestamps into raw packets
 *
 * Revision 1.36  2010/11/10 20:58:49  dechavez
 * check ISI_DL_SYS hdrlen in isidlOpenDiskLoop() to ensure existing disk loop
 * isn't incompatible. Add site name to LockDiskLoop() error messages.
 *
 * Revision 1.35  2010/11/04 21:47:30  dechavez
 * ISI_DL_SYS hdrlen support
 *
 * Revision 1.34  2009/11/13 00:37:47  dechavez
 * changed LockDiskLoop to ignore locks when lock FILE is null (ie, when creating disk loop)
 *
