#pragma ident "$Id: paro.c,v 1.12 2015/12/04 23:45:27 dechavez Exp $"
/*======================================================================
 *
...
static UINT8 crlf[2] = {0x0d, 0x0a};
    ...
    len = strlen(string);
    if ((result = ttyioWrite(pp->tty, (UINT8 *) string, len) != len)) {
...
int paroReset(PARO *pp)
{
    ...
    while(ttyioRead(pp->tty, (UINT8 *) pp->buf, 1));
...
static char *fid = "paroRead";
    ...
    synced = 0;
    while (1) {
        if ((result = ttyioRead(pp->tty, (UINT8 *) pp->buf, 1)) != 1) {
...
/* Note device identifiers */
    ...
    sprintf(want->sn, "%s", pp->param.sn);
    sprintf(want->vr, "%s", pp->param.vr);
    return TRUE;
}
 ...
 *
 * $Log: paro.c,v $
 * Revision 1.12  2015/12/04 23:45:27  dechavez
