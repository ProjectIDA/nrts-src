#pragma ident "$Id: wwfdisc.c,v 1.7 2014/04/15 19:32:13 dechavez Exp $"
/*======================================================================
 *
...
{
char segtype, clip;
static char mt_unsafe[CSSIO_WFDISC_SIZE+1];
    ...
    sprintf(buf+strlen(buf), "%8d ",      wfdisc->wfid);
    sprintf(buf+strlen(buf), "%8d ",      wfdisc->chanid);
    sprintf(buf+strlen(buf), "%8d ",      wfdisc->jdate);
    sprintf(buf+strlen(buf), "%17.5f ",   wfdisc->endtime);
    sprintf(buf+strlen(buf), "%8d ",      wfdisc->nsamp);
    ...
    sprintf(buf+strlen(buf), "%10d ",     wfdisc->foff);
    sprintf(buf+strlen(buf), "%8d ",      wfdisc->commid);
...
{
static char buffer[CSSIO_WFDISC_SIZE+1];
...
char *format = "%?.?f ";
    ...
    fprintf(fp, "%8d ",       wfdisc28->chid);
    fprintf(fp, "%8d ",       wfdisc28->wfid);
    fprintf(fp, "%-30.30s ",  wfdisc28->dir);
    fprintf(fp, "%-20.20s ",  wfdisc28->file);
    fprintf(fp, "%10d ",      wfdisc28->foff);
    fprintf(fp, "%8d ",       wfdisc28->adate);
    fprintf(fp, "%-30.30s\n", wfdisc28->remark);
 ...
 *
 * $Log: wwfdisc.c,v $
 * Revision 1.7  2014/04/15 19:32:13  dechavez
 * use MT unsafe buffer if cssioWfdiscString() is called without user supplied space
 *
 * Revision 1.6  2013/11/01 22:20:28  dechavez
 * change all %ld to %d (on 64-bit Linux this was causing the INT32s to print as garbage)
 *
 * Revision 1.5  2011/03/17 17:21:09  dechavez
