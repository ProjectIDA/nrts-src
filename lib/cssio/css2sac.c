#pragma ident "$Id: css2sac.c,v 1.10 2013/12/12 20:28:52 dechavez Exp $"
/*======================================================================
 * 
...
static INT32 ldata[BUFLEN];
static INT16 sdata[BUFLEN];
static int ascii;
static INT32 numascii = 0;
...
static int css2sac_compare(const void *aptr, const void *bptr)
{
int val;
...
static int css2sac_sacwrite(FILE *fp,float *data,INT32 npts)
{
int i;
...
static int css2sac_doi2(FILE *in, FILE *out, INT32 npts)
{
INT32 i, nread;

    while (npts > 0) {
        nread = (npts > BUFLEN) ? BUFLEN : npts;
        if (fread(sdata, sizeof(INT16), nread, in) != (size_t) nread)   return -1;
...
static int css2sac_doi2swap(FILE *in, FILE *out, INT32 npts)
{
INT32 i, nread;
...
static int css2sac_doi4(FILE *in, FILE *out, INT32 npts)
{
INT32 i, nread;

    while (npts > 0) {
        nread = (npts > BUFLEN) ? BUFLEN : npts;
        if (fread(ldata, sizeof(INT32), nread, in) != (size_t) nread)   return -1;
...
static int css2sac_doi4swap(FILE *in, FILE *out, INT32 npts)
{
INT32 i, nread;
...
static int css2sac_dof4(FILE *in, FILE *out, INT32 npts)
{
INT32 nread;
...
static int css2sac_dovftoif(FILE *in, FILE *out, INT32 npts)
{
INT32 nread;
UINT32 *rawdata;
...
static int css2sac_doiftovf(FILE *in, FILE *out, INT32 npts)
{
INT32 nread;
UINT32 *rawdata;
...
static FILE *css2sac_open(struct cssio_wfdisc *wfdisc, struct sac_header *defaults)
{
static struct cssio_wfdisc *prev = NULL;
static int count = 0;
...
FILE *out;
    ...
    if (count) sprintf(o_path+strlen(o_path), ".%05d", count);
...
static int css2sac_close(FILE *fp)
{
    sach.depmin = depmin;
    sach.depmax = depmax;
...
static int css2sac_copy(FILE *fp, struct cssio_wfdisc *wfdisc, int order)
{
FILE *ifp;
int retval, (*convert)();
...
static int css2sac_cont(struct cssio_wfdisc *prev, struct cssio_wfdisc *crnt, int tolerance)
{
...
int css2sac(struct cssio_wfdisc *wfdisc, INT32 nrec, int tolerance, struct sac_header *defaults, int doascii)
{
FILE *fp = NULL;
INT32 i;
struct cssio_wfdisc *prev;
int order;
...
/*  Make sure that the channels and datatypes are OK  */
        ...
        if (strcmp(wfdisc[i].chan, cssio_wfdisc_null.chan) == 0) {
...
/*  Sort into station/channel/time order  */
    ...
    qsort((char *) wfdisc, nrec, sizeof(struct cssio_wfdisc), css2sac_compare);


/*  Convert each wfdisc to SAC, appending continuous records  */

    for (prev = &cssio_wfdisc_null, i = 0; i < nrec; i++) {
 ...
 *
 * $Log: css2sac.c,v $
 * Revision 1.10  2013/12/12 20:28:52  dechavez
 * zero pad the numeric extension for duplicate sta.chan path
 *
 * Revision 1.9  2012/02/14 20:04:43  dechavez
 * use portable integer types (eg, long to INT32, unsigned long to UINT32),
 * made all the locally used functions static
 *
 * Revision 1.8  2011/03/17 17:46:30  dechavez
 * fixed some overlooked wfdisc_null occurences, changing them to cssio_wfdisc_null
 *
 * Revision 1.7  2011/03/17 17:21:08  dechavez
