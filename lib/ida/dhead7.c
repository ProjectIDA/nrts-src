#pragma ident "$Id: dhead7.c,v 1.6 2015/11/14 00:50:51 dechavez Exp $"
/*======================================================================
 *
...
#define BEG1HZ_OFF  18  /* begin 1-Hz time                 */
...
#define ENDEXT_OFF  46  /*   end external time             */
#define END1HZ_OFF  50  /* begin 1-Hz time                 */
...
/*  Digitizer info  */
	...
	stemp = utilUnpackINT16(src + UNIT_OFF, &stemp);
    if (stemp == 3) {
        dest->atod  = IDA_ARS;
        decim_index = 0;
    } else if (stemp == 2) {
...
/*  Year and system time multiplier  */
	...
	utilUnpackINT16(src + YEAR_OFF, &dest->beg.year);
    dest->end.year = dest->beg.year;

	utilUnpackINT16(src + MULT_OFF, &stemp);
    dest->end.mult = dest->beg.mult = (UINT32) stemp;

/*  Time tag of first datum  */

	utilUnpackUINT32(src + BEGSYS_OFF, &dest->beg.sys);
	utilUnpackUINT32(src + BEGEXT_OFF, &dest->beg.ext);
	utilUnpackUINT32(src + BEG1HZ_OFF, &dest->beg.hz);
	dest->beg.code = src[BEGQUAL_OFF];

/*  Time tag of last datum  */

	utilUnpackUINT32(src + ENDSYS_OFF, &dest->end.sys);
	utilUnpackUINT32(src + ENDEXT_OFF, &dest->end.ext);
	utilUnpackUINT32(src + END1HZ_OFF, &dest->end.hz);
	dest->end.code = src[ENDQUAL_OFF];
...
/*  Data logger format bitmap  */
    ...
    utilUnpackINT16(src + FORMAT_OFF, &dest->dl_format);

/*  Datalogger channel code  */

    utilUnpackINT16(src + CHANNEL_OFF, &dest->dl_channel);
    if (dest->dl_channel < MIN_CHAN || dest->dl_channel > MAX_CHAN) return -702;

/*  Datalogger filter code  */

    utilUnpackINT16(src + FILTER_OFF, &dest->dl_filter);
    if (dest->dl_filter < MIN_FILT || dest->dl_filter > MAX_FILT) return -703;

/*  Datalogger stream code  */

    utilUnpackINT16(src + STREAM_OFF, &dest->dl_stream);
...
/*  Number of samples  */
    ...
    utilUnpackINT16(src + NSAMP_OFF, &dest->nsamp);
 ...
 *
 * $Log: dhead7.c,v $
 * Revision 1.6  2015/11/14 00:50:51  dechavez
 * data type casts/declarations needed to calm Darwin compiles
 *
 * Revision 1.5  2012/02/14 20:07:11  dechavez
 ...
 *
 * Revision 1.4  2011/10/13 17:48:07  dechavez
 * fixed "begin" instead of "end" typos in some offset comments
 *
