#pragma ident "$Id: toida.c,v 1.8 2015/12/04 22:50:06 dechavez Exp $"
/*======================================================================
 *
...
struct buf_info {
    UINT8 *hdr;            /* pointer to header region          */
    UINT8 *dat;            /* pointer to data region            */
    UINT8 raw[IDA_BUFSIZ]; /* the whole packet                  */
};
...
struct chn_info {
    ...
    INT16 filt;           /* IDA filter code                    */
    INT16 chan;           /* IDA channel code                   */
    INT16 strm;           /* IDA stream code                    */
    INT16 maxsamp;        /* number of samples in a full buffer */
    ...
    UINT8 **out;          /* array of pointers to filled buffes */
...
static int ReverseMap(IDA *ida, char *chnloc, INT16 *pfilt, INT16 *pchan)
{
...
UINT32 i;
    ...
    for (i = 0; i < ida->chnlocmap->count; i++) {
        ...
        if (strcasecmp(test, chnloc) == 0) {
            *pfilt = (INT16) map->fcode;
            *pchan = (INT16) map->ccode;
            return 0;
        }
    }
...
static char *fid = "seed_toidainit";
    ...
    for (i = 0; i < info->nsta; i++) {
        ...
        for (j = 0; j < sta->nchn; j++) {
            ...
            if ((chn->out = (UINT8 **) malloc(sizeof(UINT8 *)*nbuf)) == NULL) {
                util_log(1, "%s: malloc: %s", fid, syserrmsg(errno));
                return (void *) NULL;
            }
            chn->maxsamp   = dlen / sizeof(INT32);
            chn->nsamp     = 0;
...
static struct chn_info *get_strm(char *sname, char *cname, struct info *info)
{
struct sta_info *sta;
struct chn_info *chn;
int i, j;
...
static void idarev8(char *packet, struct chn_info *strm, double tofs, double sint)
{
double tols;
UINT16 sval, mult = 1;
UINT32 lval, hz, sys, ext, tmp;
...
/* Check for time increment errors */
    ...
    if (strm->nsamp > 0) {
        error = (int) labs((INT32)((seed->beg-strm->expected)/seed->sint));
...
/* Loop over all samples in the input packet */
    ...
    while (rem) {
        if (strm->nsamp == 0) idarev8((char *) hdr, strm, tons, seed->sint);
        ptr = (UINT8 *) dat + (strm->nsamp * sizeof(INT32));
        need = strm->maxsamp - strm->nsamp;
        ncopy = need < rem ? need : rem;
        nbytes = ncopy * sizeof(INT32);
        ...
        if (strm->swap) util_lswap((INT32 *) ptr, ncopy);
...
static char *defspec = "/usr/nrts/etc";
    ...
    seed.data = (INT32 *) malloc(seed.datlen * sizeof(INT32));
    if (buffer == (UINT8 *) NULL || seed.data == (INT32 *) NULL) {
        perror("malloc");
        exit(1);
    }
 ...
 *
 * $Log: toida.c,v $
 * Revision 1.8  2015/12/04 22:50:06  dechavez
 * casts and format fixes to calm OS X compiles
 *
 * Revision 1.7  2012/02/14 20:22:07  dechavez
