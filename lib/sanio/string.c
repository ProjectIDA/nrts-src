#pragma ident "$Id: string.c,v 1.5 2015/12/04 22:55:16 dechavez Exp $"
/*======================================================================
 *
 *  SAN related strings
 *
...
char *sanioMessageTypeToString(int code)
{
int i;
static char unknown[] = "UnknownMessageCode=0xXXXXXXXX and slop";
    ...
    while (MesgTable[i].text != NULL) {
        if (MesgTable[i].code == code) return MesgTable[i].text;
    }
    sprintf(unknown, "UnknownMessageCode=0x%08x", code);
    return unknown;
}
 ...
 *
 * $Log: string.c,v $
 * Revision 1.5  2015/12/04 22:55:16  dechavez
 * fixed = instead of == bug in sanioMessageTypeToString()
 *
