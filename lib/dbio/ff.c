#pragma ident "$Id: ff.c,v 1.16 2015/11/04 22:31:25 dechavez Exp $"
/*======================================================================
 *
...
/*  Read again, loading entries into map structure  */
    ...
    /*  Rest is description  */
        ...
        slen = strlen(token) + 1;
        if ((ff->rev[i].text = malloc(slen)) == NULL) return CloseFp(fp, -5);
        strlcpy(ff->rev[i].text, token, slen);
...
static int LoadIMEI(IDAFF *ff, char *path, char *buffer)
{
FILE *fp;
int i, status;
int lineno = 0;
char *token[5];
static char *fid = "idaffInit:LoadIMEI";

/* Don't care if this file does not exist */
...
/*  Read once to count how many entries it has and allocate for them  */
    ...
    ff->nimei = 0;
    while ((status = GETLINE('#')) == 0) ++ff->nimei;
    if (status != 1) return CloseFp(fp, -2);

    if ((ff->imei = (IDAFF_IMEI *) malloc(ff->nimei*sizeof(IDAFF_IMEI))) == NULL) return CloseFp(fp, -3);
...
/*  Read again, loading entries into map structure  */
    ...
    i = 0;
    while ((status = GETLINE('#')) == 0) {
        if (util_sparse(buffer, token, " ", 4) != 4) {
            util_log(1, "%s: error at %s line %d", fid, path, lineno);
            return CloseFp(fp, -4);
        }
...
char *isicfg;
    ...
    if (utilFileExists(path) && LoadSint(ff, path, buf) != 0) return FALSE;
...
BOOL idaffLookupSiteByIMEI(IDAFF *ff, char *imei, char *site, char *net, UINT64 *zerotime)
{
int i;

    if (ff == (IDAFF *) NULL || imei == NULL || site == NULL || net == NULL || zerotime == NULL) {
        errno = EINVAL;
        return FALSE;
    }

    for (i = 0; i < ff->nimei; i++) if (strcmp(ff->imei[i].imei, imei) == 0) {
        strlcpy(site, ff->imei[i].site, IDA_SNAMLEN+1);
        strlcpy(net,  ff->imei[i].net,  IDA_NNAMLEN+1);
        *zerotime = ff->imei[i].zerotime;
        return TRUE;
    }
...
char *idaffLookupIMEIbySite(IDAFF *ff, char *site)
{
int i;
    ...
    for (i = 0; i < ff->nimei; i++) if (strcmp(ff->imei[i].site, site) == 0) return ff->imei[i].imei;
 ...
 *
 * $Log: ff.c,v $
 * Revision 1.16  2015/11/04 22:31:25  dechavez
 * cosmetic stuff to calm OS X compiler
 *
 * Revision 1.15  2013/05/11 22:56:53  dechavez
 * added IMEI map support
 *
