#pragma ident "$Id: parse.c,v 1.13 2012/12/13 21:48:17 dechavez Exp $"
/*======================================================================
 *
 *  parse strings
 *
...
int ntoken, port;
    ...
    ntoken = utilParse(string, token, ":", MAXTOKEN, 0);
    if (ntoken < 1) return FALSE;

    if (ntoken < 1 || ntoken > 2) {
        errno = EINVAL;
        return FALSE;
    }

    strncpy(pserver, token[0], MAXPATHLEN);

    if (ntoken == 2) {
        if ((port = atoi(token[1])) <= 0) return FALSE;
        *pport = port;
    }
 ...
 *
 * $Log: parse.c,v $
 * Revision 1.13  2012/12/13 21:48:17  dechavez
 * changed benign but incorrect return FALSE to return NULL in utilStringTokenList()
 * removed some tabs
 *
 * Revision 1.12  2012/04/25 21:05:43  dechavez
 * added utilParseServer()
 *
