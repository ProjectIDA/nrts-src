#pragma ident "$Id: steim.c,v 1.7 2014/04/14 18:15:56 dechavez Exp $"
/*======================================================================
 *
...
INT32 *out;   /* output buffer                                     */
INT32 *frame; /* current frame                                     */
INT32 fi;     /* index of current frame                            */
INT32 maxfrm; /* number of frames that will fit into output buffer */
int  si;     /* sequence index in current frame                   */
INT32 nc;     /* number of compressed samples in output            */
INT32 ocount; /* number of bytes used in output buffer             */
...
static int csteim1_addword(INT32 *d, int ndiff)
{
union {
    INT8  *c;
    INT16 *s;
...
/* Load in the differences and associated descriptor */
    ...
    ptr.l = frame + si;
    if (ndiff == 4) {
        ptr.c[0]  = (UINT8) (d[0] & 0xff);
        ptr.c[1]  = (UINT8) (d[1] & 0xff);
        ptr.c[2]  = (UINT8) (d[2] & 0xff);
        ptr.c[3]  = (UINT8) (d[3] & 0xff);
    } else if (ndiff == 2) {
        ptr.s[0]  = (INT16) (d[0] & 0xffff);
        ptr.s[1]  = (INT16) (d[1] & 0xffff);
...
static int csteim1_frm(INT32 diff, int flush)
{
/* Possible states */
...
/* UINT8 *dest   => output array                         */
/* INT32 destlen => size of dest, in bytes               */
/* INT32 *src    => input array                          */
/* INT32 count   => number of entries in src             */
/* INT32 *used   => number of output bytes actually used */
...
/* Loop for each sample (will bail when we run out of output memory) */
    ...
    for (s = 0; s < count; s++) {
        ...
        if (csteim1_frm(diff, 0) != 0) {
            if (used != (INT32 *) NULL) *used = ocount;
            if (swap) {
                util_lswap(out+1, 2);
            }
            return nc;
...
static int read_INT16(FILE *fp, INT32 *sample, int *done)
{
INT16 sval;

    if (fread(&sval, sizeof(INT16), 1, fp) != 1) {
        *done = 1;
        return -1;
    }
    *sample = (INT32) sval;
...
static int read_INT32(FILE *fp, INT32 *sample, int *done)
{
    if (fread(sample, sizeof(INT32), 1, fp) != 1) {
        *done = 1;
        return -1;
    }
    return 0;
...
static int read_float(FILE *fp, INT32 *sample, int *done)
{
float fval;
...
static int read_double(FILE *fp, INT32 *sample, int *done)
{
double dval;
...
static int read_float_gain(FILE *fp, INT32 *sample, int *done)
{
float fval;
    ...
    *sample = (INT32) (fval * float_factor);
    return 0;
}

static int read_double_gain(FILE *fp, INT32 *sample, int *done)
{
double dval;
    ...
    *sample = (INT32) (dval * double_factor);
    return 0;
}
...
/* Figure out how to read the input */
    ...
    switch (type) {
      case SEED_INT_16:
        get_sample = read_INT16;
        break;
      case SEED_INT_32:
        get_sample = read_INT32;
        break;
      case SEED_IEEE_F:
        if (gain == (double) 1.0) {
...
/* INT32 *dest   => output array                            */
/* INT32 destlen => dimension of output array               */
/* UINT8 *src    => input Steim 1 compressed data           */
/* INT32 srclen  => size of src, in bytes                   */
/* UINT32 order => byte order (as per util_order()) of src */
/* INT32 count   => number of uncompressed samples in src   */
...
/* We do not verify that the 0x00 codes are where they should be */
    ...
    for (i = 0; i < nfrm; i++) {
        ...
        key = *((INT32 *) frm);   /* codes are in first 4 bytes      */
        ...
        for (j = 1; j < NSEQ; j++) {
            ...
            ptr.c = (INT8 *) (frm + (j * 4));  /* point to current 4 byte sequence */
...
/* INT32 *dest   => output array                            */
/* INT32 destlen => dimension of output array               */
/* UINT8 *src    => input Steim 2 compressed data           */
/* INT32 srclen  => size of src, in bytes                   */
/* UINT32 order  => byte order (as per util_order()) of src */
/* INT32 count   => number of uncompressed samples in src   */
...
INT32 new[NEWLEN], used;
    ...
    printf("\n");
    util_hexdmp((UINT8 *) output, OUTLEN, 0, 'd');
    printf("\n");
 ...
 *
 * $Log: steim.c,v $
 * Revision 1.7  2014/04/14 18:15:56  dechavez
 * at some time I added some explicit type casts... don't see they make any difference.  Committing the beast anyway.
 *
 * Revision 1.6  2012/02/14 19:55:02  dechavez
