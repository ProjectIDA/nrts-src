#pragma ident "$Id: list.c,v 1.13 2015/12/04 22:38:51 dechavez Exp $"
/*======================================================================
 *
...
VOID listPrintNode(LNKLST_NODE *node)
{
    ...
    printf(" length=%u",     node->length);
    printf(" payload=%p", node->payload);
    printf(" parent=%p",  node->payload);
    printf("\n");
}
...
LNKLST_NODE *listUnlinkNode(LNKLST_NODE *crnt)
{
LNKLST *parent;
LNKLST_NODE *next;
...
LNKLST_NODE *next;
    ...
    next = listNextNode(crnt); /* listNextNode() returns NULL if no more */

    /* excise current node from the list, and destroy it */

    if ((crnt = listUnlinkNode(crnt)) == NULL) return NULL;
...
LNKLST_NODE *crnt;
    ...
    crnt = listFirstNode(list);
    while (crnt != NULL) {
        fprintf(fp, "%lu: len=%lu\n", (unsigned long) count++, (unsigned long) crnt->length);
 ...
 *
 * $Log: list.c,v $
 * Revision 1.13  2015/12/04 22:38:51  dechavez
 * use %p to print pointers
 *
 * Revision 1.12  2015/11/13 21:16:20  dechavez
 * cast some [s]printf() arguments to calm Darwin compiles
 *
 * Revision 1.11  2013/02/07 17:58:19  dechavez
 * added listUnlinkNode() and listCount() (lightly tested, I think they work)
 *
