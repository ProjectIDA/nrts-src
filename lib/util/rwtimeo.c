#pragma ident "$Id: rwtimeo.c,v 1.9 2015/12/22 21:37:35 dechavez Exp $"
/*======================================================================
 *
...
/*  Write to fd until no more can be written  */
    ...
    ptr  = buffer;
    while (nrem > 0) {
        error = select(FD_SETSIZE, NULL, &writefds, NULL, &timeout);
        if (error == 0) {
            errno = ETIMEDOUT;
            return n - nrem;
        } else if (error < 0) {
 ...
 *
 * $Log: rwtimeo.c,v $
 * Revision 1.9  2015/12/22 21:37:35  dechavez
 * use FD_SETSIZE for width in all select() calls.
 * Fixes all sorts of weird downstream problems in FreeBSD 10.1
 *
