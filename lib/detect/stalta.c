#pragma ident "$Id: stalta.c,v 1.3 2015/11/04 22:49:37 dechavez Exp $"
/*======================================================================
 * 
...
static char MtUnsafeSpace[1000];
    ...
    sprintf(buf, " STA=%u LTA=%u STA/LTA=%.2f", stalta->trigger.sta, stalta->trigger.lta, stalta->trigger.ratio);
    return buf;
}
...
{
    if ((window->buffer = (INT32 *) calloc(len, sizeof(INT32))) == NULL) return FALSE;
    window->len = len;
    window->sum = 0;
    window->index = 0;
 ...
 *
 * $Log: stalta.c,v $
 * Revision 1.3  2015/11/04 22:49:37  dechavez
 * cosmetic changes to calm OS X compiler
 *
