#pragma ident "$Id: meta.c,v 1.11 2016/01/28 00:55:10 dechavez Exp $"
/*======================================================================
 *
...
static char *fid = "qdplusReadMetaData:LoadMetaDataDirectory:LoadMetaDatum";
...
static BOOL ValidFile(char *path, char *fname)
{
    ...
    if (strlen(fname) != ISI_SEQNO_STRING_LEN) return FALSE;
...
LNKLST *dirlist;
QDPLUS_META_DATUM *datum;
static char *fid = "qdplusReadMetaData:LoadMetaDataDirectory";
...
QDPLUS_META_DATUM *datum;
    ...
    if ((datum = LocateMetaDatum(head, serialno, seqno)) != NULL) {
        ...
        memcpy(lcq->meta.raw.epd, datum->data.raw.epd, QDP_MAX_PAYLOAD);
        qdpDecode_C2_EPD(datum->data.raw.epd, &lcq->meta.epd);
        lcq->meta.state |= QDP_META_STATE_HAVE_EPD;
    }
 ...
 *
 * $Log: meta.c,v $
 * Revision 1.11  2016/01/28 00:55:10  dechavez
 * add C2_EPD supprot to qdplusInitializeLCQMetaData()
 *
 * Revision 1.10  2009/10/02 22:33:05  dechavez
 * fixed ValidFile() bug failing to detect illegal length file names in the meta directory
 *
