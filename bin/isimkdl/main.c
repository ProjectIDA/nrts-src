#pragma ident "$Id: main.c,v 1.21 2014/08/28 21:47:41 dechavez Exp $"
/*======================================================================
 *
...
#define DEFAULT_REFTEK_NREC   489384 /* need to figure out good default value */
#define DEFAULT_REFTEK_NHIDE  17568 /* need to figure out good default value */
#define DEFAULT_REFTEK_RECLEN 1024

#define DEFAULT_FEMTO_NREC   120000 /* a lot */
...
char marker[MAXPATHLEN+1];
    ...
    for (i = 1; i < argc; i++) {
        if (strncmp(argv[i], "db=", strlen("db=")) == 0) {
            ...
            } else if (strcmp(string, "reftek") == 0) {
                type = TYPE_REFTEK;
                nrec = DEFAULT_REFTEK_NREC;
                nhide = DEFAULT_REFTEK_NHIDE;
                reclen = DEFAULT_REFTEK_RECLEN;
            } else if (strcmp(string, "femto") == 0) {
                type = TYPE_FEMTO;
...
/* Check to see if there already is a disk loop, by trying to open it */
    ...
    if ((dl = isidlOpenDiskLoop(&glob, sta, &lp, ISI_RDONLY, ISI_OPTION_NONE)) != NULL) {
...
/* QDP disk loops need a subdirectory for meta-data */
    ...
    if (type == TYPE_QDP) {
        ...
        isidlCloseDiskLoop(dl);
        isidlOpenDiskLoop(&glob, sta, &lp, ISI_RDONLY, ISI_OPTION_NONE);
    }

/* Reftek disk loops need a marker so I can keep them out of some scripts */

    if (type == TYPE_REFTEK) {
        sprintf(marker, "%s/.reftek", dl->base);
        if (!utilCreateFile(marker, 0, FALSE, 0)) {
            fprintf(stderr, "ERROR: can't create reftek tag: ");
            perror(marker);
            exit(1);
        }
    }
 ...
 *
 * $Log: main.c,v $
 * Revision 1.21  2014/08/28 21:47:41  dechavez
 * include option flags as isidlOpenDiskLoopSet() argument
 *
 * Revision 1.20  2013/05/11 22:58:57  dechavez
 * added type "addoss"
 *
 * Revision 1.19  2013/03/07 21:16:00  dechavez
 * added support for type 'sbd'
 *
 * Revision 1.18  2011/01/25 21:35:44  dechavez
 * added femto option
 *
 * Revision 1.17  2010/01/26 22:35:49  dechavez
