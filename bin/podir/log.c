#pragma ident "$Id: log.c,v 1.6 2015/09/30 20:25:45 dechavez Exp $"
/*======================================================================
 *
 *  Logging facility
 *
...
char message[MAXPATHLEN+1];
    ...
    LogMsg(LogLevel, "working directory: %s\n", workdir);
    LogMsg(LogLevel, "database:          %s\n", dbpath);
    LogMsg(LogLevel, "excluded stations: %s\n", StationSelectionString(message, MAXPATHLEN));
}
...
static char *fid = "InitLogging";
    ...
    LogMsg(LogLevel, "%s - %s", myname, VersionIdentString);
    LogMsg(LogLevel, "Build %s %s", __DATE__, __TIME__);
 ...
 *
 * $Log: log.c,v $
 * Revision 1.6  2015/09/30 20:25:45  dechavez
