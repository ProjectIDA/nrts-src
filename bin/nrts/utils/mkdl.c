#pragma ident "$Id: mkdl.c,v 1.11 2015/12/08 21:32:19 dechavez Exp $"
/*======================================================================
 *
...
/*  See if we already have a disk buffer  */
    ...
    if (stat(file->hdr, &hstat) != 0) {
        if (errno == ENOENT) {
            ...
            fprintf(stderr, "can't stat %s", file->hdr);
            perror(file->hdr);
            exit(1);
        }
    } else {
    ...
    if (stat(file->dat, &dstat) != 0) {
        if (errno == ENOENT) {
            ...
            fprintf(stderr, "can't stat %s", file->dat);
            perror(file->dat);
            exit(1);
        }
    } else {
    ...
    if (stat(file->sys, &dstat) != 0) {
        if (errno == ENOENT) {
            ...
            fprintf(stderr, "can't stat %s", file->sys);
            perror(file->sys);
            exit(1);
        }
    } else {
 ...
 *
 * $Log: mkdl.c,v $
 * Revision 1.11  2015/12/08 21:32:19  dechavez
 * fixed some printf format strings, error finally caught by Apple's clang
 *
 * Revision 1.10  2011/02/15 23:18:25  dechavez
 * eliminate uneeded prints, link with libnrts 4.11.0 for per-channel packet length override
 *
