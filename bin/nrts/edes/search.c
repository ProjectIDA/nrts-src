#pragma ident "$Id: search.c,v 1.5 2015/12/09 18:36:06 dechavez Exp $"
/*======================================================================
 *
...
static INT32 do_search(struct edes_params *edes, int i, int j, int k, double target, INT32 left, INT32 rite, char *buffer)
{
INT32 test;
...
/* Second case, data wrap around end of disk buffer */
    ...
    span = get_span(edes, i, j, k, (INT32) 0, buffer);
    ...
    if (target > span->beg && target <= dl.end) {
        ...
        span = get_span(edes, i, j, k, (INT32) CHN->hdr.nrec - 1, buffer);
    ...
    return unreached; /* to calm certain compilers */
}

/* Revision History
 *
 * $Log: search.c,v $
 * Revision 1.5  2015/12/09 18:36:06  dechavez
 * various minor tweaks to get clean Mac OS X builds
 *
 * Revision 1.4  2012/02/14 21:33:09  dechavez
