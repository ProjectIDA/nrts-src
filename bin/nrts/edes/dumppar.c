#pragma ident "$Id: dumppar.c,v 1.2 2015/12/09 18:36:06 dechavez Exp $"
/*======================================================================
 *
...
static void dump_inf_chn(int level, struct edes_inf_chn *chn)
{
    ...
    sprintf(buf+strlen(buf), "begndx=%d ", chn->begndx);
    sprintf(buf+strlen(buf), "endndx=%d ", chn->endndx);
    sprintf(buf+strlen(buf), "nxtndx=%d ", chn->nxtndx);
    sprintf(buf+strlen(buf), "count=%d ",  chn->count);
 ...
 *
 * $Log: dumppar.c,v $
 * Revision 1.2  2015/12/09 18:36:06  dechavez
