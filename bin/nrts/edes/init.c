#pragma ident "$Id: init.c,v 1.14 2015/08/24 19:17:19 dechavez Exp $"
/*======================================================================
 *
...
/*  Start logging facility */
    ...
    if (peer != NULL) {
        if (NotOnSilentList(peer)) {
            ...
            util_log(1, "Connection from %s", PeerString);
        }
    } else {
        util_logopen(log, 1, NRTS_MAXLOG, debug, NULL, argv[0]);
        util_log(1, VersionIdentString);
        util_log(1, "Connection from <can't determine peer string>\n");
    }
 ...
 *
 * $Log: init.c,v $
 * Revision 1.14  2015/08/24 19:17:19  dechavez
 * print connections with log level 1
 *
