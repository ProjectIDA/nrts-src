#pragma ident "$Id: seqno.c,v 1.29 2015/12/08 18:41:55 dechavez Exp $"
/*======================================================================
 *
...
/* everything is selected if there is no control list */

    selected = (datreq->slist->count == 0) ? TRUE : FALSE; /* everything selected if no list */

/* otherwise, check the list for a match */

    if (!selected) {
        ida = dl->nrts == NULL ? NULL : dl->nrts->ida;
        if (isiRawPacketStreamName(&name, raw, ida) == NULL) {
            selected = TRUE; /* non-data packets automatically selected */
        } else {
            strlcpy(name.sta, dl->sys->site, ISI_STALEN+1);
        }

        crnt = listFirstNode(datreq->slist);
        while (!selected && crnt != NULL) {
            if (isiStreamNameCompare(&name, (ISI_STREAM_NAME *) crnt->payload) == 0) selected = TRUE;
            crnt = listNextNode(crnt);
        }
    }

/* when requesting MiniSEED, ensure packet can in fact be delivered in that format */

    if (selected && datreq->format == ISI_FORMAT_MSEED) selected = (isiRawToMiniSeed(raw) == ISI_TYPE_MSEED) ? TRUE : FALSE;
...
/* Verify disk loop for requested site exists */
    ...
    for (i = 0; i < datreq->nreq; i++) {
        ...
        if ((sreq->dl = isidlLocateDiskLoopBySite(client->master, sreq->site)) == NULL) {
...
/* (Also, if requesting MiniSEED OkToSend will only be TRUE for MiniSEED payloads) */

    if (OkToSend(datreq, raw, sreq->dl)) {

    /* Compress (native packets) */

        if (datreq->format == ISI_FORMAT_NATIVE && datreq->compress != ISI_COMP_NONE) {
            ...
            if (!isiCompressRawPacket(raw, compressedBuf, cbuflen)) {
                LogMsg(LOG_WARN, "%s: isiCompressRawPacket: %s", fid, strerror(errno));
            }
        }
    ...
    /* Package it into an IACP_FRAME */
        ...
        switch (datreq->format) {
          case ISI_FORMAT_NATIVE:
            ...
            break;
          case ISI_FORMAT_MSEED:
            frame->payload.type = ISI_IACP_MSEED;
            memcpy(frame->payload.data, raw->payload, raw->hdr.len.used);
            frame->payload.len = raw->hdr.len.used;
            break;
          default:
            LogMsg(LOG_ERR, "%s: UNEXPECTED FORMAT (%s): %s", fid, isiFormatString(datreq->format));
            client->result = IACP_ALERT_SERVER_FAULT;
            client->finished = TRUE;
            return;
        }
 ...
 *
 * $Log: seqno.c,v $
 * Revision 1.29  2015/12/08 18:41:55  dechavez
 * simplified error message after isiCompressRawPacket() failure in ReadAndForward() to calm DARWIN builds
 *
 * Revision 1.28  2012/06/27 15:34:57  dechavez
 * isidlLocateDiskLoopBySite() instead of (formerly) local LocateDiskLoopBySite
 *
 * Revision 1.27  2011/11/07 17:51:40  dechavez
 * moved RawToMiniSeed() to new library routine isiRawToMiniSeed()
 *
 * Revision 1.26  2011/11/03 18:01:26  dechavez
 * Support for ISI_FORMAT_MSEED seqno requests
 *
