#pragma ident "$Id: report.c,v 1.7 2015/12/08 18:43:19 dechavez Exp $"
/*======================================================================
 *
...
time_t start;
    ...
    fprintf(fp, "Client index %u\n", client->index);
    fprintf(fp, "%s connected since %s ", client->ident, utilLttostr(start, 0, buf));
    fprintf(fp, "(%s)\n", utilLttostr(now - start, 8, buf));

    fprintf(fp, "Last input:   %s (%u bytes, %u frames) (last type=%u, len=%u)\n",
        utilLttostr(now - recv.tstamp, 8, buf), recv.stats.nbyte, recv.stats.nframe, client->last.msg.type, client->last.msg.len
    );

    fprintf(fp, "Last output:  %s (%u bytes, %u frames)\n",
 ...
 *
 * $Log: report.c,v $
 * Revision 1.7  2015/12/08 18:43:19  dechavez
 * fixed up some print format strings to calm DARWIN builds
 *
