#pragma ident "$Id: convert.c,v 1.19 2014/04/18 21:21:12 dechavez Exp $"
/*======================================================================
 *
...
long *ldata;
    ...
    if (pkt->work.buf.ida.head.ffu_shift > 0) {
        ldata = (long *) (src);
        for (i = 0; i < pkt->isi.hdr.nsamp ; i++) ldata[i] >>= pkt->work.buf.ida.head.ffu_shift; 
     }
}
...
/* We can only handle uncompressed INT32's here */
    ...
    if (pkt->isi.hdr.desc.type == ISI_TYPE_REAL32) {
        if (pkt->isi.hdr.desc.order != BIG_ENDIAN_BYTE_ORDER) {
            utilSwapUINT32((UINT32 *) pkt->isi.data, pkt->isi.hdr.nsamp);
            pkt->isi.hdr.desc.order = BIG_ENDIAN_BYTE_ORDER;
        }
        return UNSUPPORTED_COMPRESSION;
    }

    if (pkt->isi.hdr.desc.type == ISI_TYPE_REAL64) {
        if (pkt->isi.hdr.desc.order != BIG_ENDIAN_BYTE_ORDER) {
            utilSwapUINT64((UINT64 *) pkt->isi.data, pkt->isi.hdr.nsamp);
            pkt->isi.hdr.desc.order = BIG_ENDIAN_BYTE_ORDER;
        }
        return UNSUPPORTED_COMPRESSION;
    }
...
/* Silently apply Fels' shifts to IDA5 and 6 data */
    ...
    if (pkt->orig.type == ISI_TYPE_IDA5 || pkt->orig.type == ISI_TYPE_IDA6) ApplyFFU(pkt, pkt->isi.data);

/* Compress */

    newlen = utilIdaCompressINT32(dest, (INT32 *) pkt->isi.data, pkt->isi.hdr.nsamp);
 ...
 *
 * $Log: convert.c,v $
 * Revision 1.19  2014/04/18 21:21:12  dechavez
 * removed some host byte order manipulations that ended up introducing a byte order
 * bug in some types of served data, after I fixed a bug in libnrts that was what made
 * me put said host byte order manipulations here.  I should stop programming sooner
 * rather than later... clearly past my prime now.
 *
 * Revision 1.18  2013/01/18 23:38:41  dechavez
 * accomodate change of NRTS_PKT workspace for IDA9 from IDA_DHEAD to IDA_DREC (libnrts 4.15.0)
 *
 * Revision 1.17  2012/12/20 20:20:06  dechavez
 * Fix byte order problem when serving generic REAL32/REAL64 data from little-endian servers
 *
