#pragma ident "$Id: par.c,v 1.20 2016/08/04 22:36:31 dechavez Exp $"
/*======================================================================
 *
...
static VOID Fail(char *fid, FILE *fp, char *path, char *token, UINT32 lineno)
{
    ...
    fprintf(stderr, "%u, file `%s', token `%s'\n", lineno, path, token);
    fclose(fp);
    exit(MY_MOD_ID + 1);
}
...
/*  Load the global initialization */
    ...
    if (!isidlSetGlobalParameters(dbspec, prog, &par.glob)) {
        fprintf(stderr, "%s: isidlSetGlobalParameters(%s): %s\n", prog, dbspec, strerror(errno));
        exit(1);
    }
 ...
 *
 * $Log: par.c,v $
 * Revision 1.20  2016/08/04 22:36:31  dechavez
 * include dbspec in error message when unable to load database
 *
 * Revision 1.19  2015/12/08 18:44:22  dechavez
 * fixed up some print format strings to calm DARWIN builds
 *
 * Revision 1.18  2012/05/30 21:15:27  dechavez
