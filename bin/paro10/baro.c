#pragma ident "$Id: baro.c,v 1.3 2014/08/28 23:14:50 dechavez Exp $"
/*======================================================================
 *
...
#define MY_MOD_ID PARO10_MOD_BARO

static BOOL debug = FALSE;
static char SerialNumber[PARO_MAXCHAR+1];

void LogBarometerSerialNumber()
{
    LogMsg(LOG_INFO, "ParoScientific barometer S/N %s\n", SerialNumber);
}
...
static char *fid = "GetSample";
    ...
    MUTEX_LOCK(&bp->mutex);
        if (bp->ValidSampleSeen) {
            value = bp->sample;
            bp->sample = PARO10_MISSED_SAMPLE;
        } else {
            value = PARO10_NO_SENSOR;
        }
    MUTEX_UNLOCK(&bp->mutex);
...
/* connect to barometer and verify constants */
    ...
    strncpy(SerialNumber, bp->pp->param.sn, PARO_MAXCHAR);
 ...
 *
 * $Log: baro.c,v $
 * Revision 1.3  2014/08/28 23:14:50  dechavez
 * added LogBarometerSerialNumber()
 *
