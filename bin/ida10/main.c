#pragma ident "$Id: main.c,v 1.18 2017/10/20 01:09:01 dauerbach Exp $"
/*======================================================================
 *
...
#define CASCADE(i, count) (config->stream[i].dsp.cascade[count])
    ...
    printf("\n");
    printf("%d filters:\n", config->nfilter);
    printf( "Id  # Taps    Decim    First coeff      " "Mid Coeff        Last Coeff\n");
    for (i = 0; i < config->nfilter; i++) {
        last = config->filter[i].ntaps - 1;
        mid  = (config->filter[i].ntaps / 2);
        printf("%2d ", config->filter[i].fid);
        printf("%5d ", config->filter[i].ntaps);
        printf("%8u ", config->filter[i].decim);
    ...
    for (i = 0; i < config->nstream; i++) {
        ...
        if (config->stream[i].dsp.chan < 0) {
            ...
            printf("%5d ", config->stream[i].dsp.chan);
            printf("%4d ", config->stream[i].dsp.flags);
            tmpbuf[0] = 0;
            count = 0;
            while (count < 16 && CASCADE(i, count) != -1) {
                if (strlen(tmpbuf) != 0) sprintf(tmpbuf+strlen(tmpbuf), ",");
                sprintf(tmpbuf+strlen(tmpbuf), "%d", CASCADE(i, count));
                ++count;
            }
...
{
    utilPrintHexDump(stdout, buf, 64);
}
...
"sname, cname, tofs, tqual, tols, tqual, sint, gain, nsamp, unused [seqno host_time]\n"
...
"10.0 through 10.2 are MK8 time tags, 10.3 is the femtometer, and 10.4 the Q330.\n";
    ...
    fprintf(stderr, "usage: %s [-v | -t] [ -noseqno -tshdr ] < stdin > stdout\n", myname);
    fprintf(stderr, "\n");
    fprintf(stderr, "-noseqno - suppress sequence number continuity checks\n");
    fprintf(stderr, "-tshdr   - input consists of TS headers only\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "%s\n", details);
...
BOOL TSheaders = FALSE;
    ...
    recno = 0;
    while ((nread = ida10ReadGz(gz, buf, IDA10_MAXRECLEN, &RecordType, TSheaders)) > 0) {
        ++recno;
    ...
    status = nread;
    if (status != IDA10_EOF) printf("ida10ReadGz failed at record no. %ld: %s\n", recno, ida10ErrorString(status));
 ...
 *
 * $Log: main.c,v $
 * Revision 1.18  2017/10/20 01:09:01  dauerbach
 * have rawDump call utilPrintHexDump
 *
 * Revision 1.17  2015/12/08 20:58:53  dechavez
 * various minor tweaks to get clean Mac OS X builds
 *
 * Revision 1.16  2015/06/18 00:18:24  dechavez
 * changed help message to reflect new contents of verbose string (ie, clock qual)
 *
 * Revision 1.15  2011/02/25 19:42:40  dechavez
 * -hdr option for reading header only files
 *
 * Revision 1.14  2011/02/24 17:24:21  dechavez
 * use gzFile and ida10ReadGz() for input
 *
