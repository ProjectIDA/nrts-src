#pragma ident "$Id: main.c,v 1.35 2015/12/08 18:49:57 dechavez Exp $"
/*======================================================================
 *
...
static ISI_DATA_REQUEST *BuildRawRequest(int compress, int format, ISI_SEQNO *begseqno, ISI_SEQNO *endseqno, char *SiteSpec)
{
ISI_DATA_REQUEST *dreq;
...
static BOOL ReadFrame(ISI *isi, int want)
{
int status;

    while ((status = isiReadFrame(isi, TRUE)) == ISI_OK) {
        if (isi->frame.payload.type == want) return TRUE;
...
static void WritePayload(char *site, UINT8 *payload, int len)
{
FILE *fp;
...
static void rawdata(char *server, ISI_PARAM *par, int compress, int format, ISI_SEQNO *begseqno, ISI_SEQNO *endseqno, char *SiteSpec)
{
ISI *isi;
ISI_RAW_PACKET raw;
ISI_DATA_REQUEST *dreq;
...
LISS_PKT liss;
    ...
    dreq = BuildRawRequest(compress, format, begseqno, endseqno, SiteSpec);
    ...
    if (format == ISI_FORMAT_NATIVE) {
        while (ReadFrame(isi, ISI_IACP_RAW_PKT)) {
            ++count;
            isiUnpackRawPacket(isi->frame.payload.data, &raw);
            if (TERSE) fprintf(stderr, "%s\n", isiRawHeaderString(&raw.hdr, (char *) buf));
            ...
            if (datdir != NULL) WritePayload(raw.hdr.site, raw.payload, raw.hdr.len.used);
#ifdef INCLUDE_DL_OPTION
            if (dl != NULL) isidlWriteToDiskLoop(dl, &raw, 0);
#endif /* INCLUDE_DL_OPTION */
        }
    } else if (format == ISI_FORMAT_MSEED) {
        while (ReadFrame(isi, ISI_IACP_MSEED)) {
            ++count;
            lissUnpackMiniSeed(&liss, isi->frame.payload.data, 0);
            if (TERSE) fprintf(stderr, "%s\n", lissMiniSeedHdrString(&liss.hdr, (char *) buf));
            if (VERBOSE) utilPrintHexDump(stderr, isi->frame.payload.data, 64);
            if (datdir != NULL) WritePayload(liss.hdr.fsdh.staid, isi->frame.payload.data, isi->frame.payload.len);
        }
    }
}
...
static void DisplayDlReport(ISI_REPORT *report)
{
int i;
ISI_DL_SYS sys;
...
char buf[BUFLEN];
    ...
    if (VERBOSE) printf(
        "  site    uptime      update              seqno           count     status"
    );

    for (i = 0; i < (int) report->nentry; i++) {
        entry = (ISI_REPORT_ENTRY *) &report->entry[i];
        if (entry->type != ISI_REPORT_TYPE_DLSYS) {
            fprintf(stderr, "*** ERROR *** expected report type %d, but got %d!\n", ISI_REPORT_TYPE_DLSYS, entry->type);
            exit(1);
        }
        isidlUnpackDLSys(entry->payload, &sys);
        printf("%7s ", sys.site);
        printf("%s ", utilTimeString(sys.tstamp.start, 108, buf, BUFLEN));
        printf("%s ", utilTimeString(sys.tstamp.write, 108, buf, BUFLEN));
        printf("%s ", isiSeqnoString2(&sys.seqno, buf));
        printf("%9llu ", sys.count);
        printf("%s\n", isidlStateString(sys.state));
    }
}

static void dl(char *server, ISI_PARAM *par)
{
ISI_REPORT *report;

    if ((report = isiReport(server, par, ISI_REPORT_TYPE_DLSYS)) == NULL) {
        perror("isiReport");
        exit(1);
    }

    DisplayDlReport(report);
    isiFreeReport(report);
}

#ifdef notdef
static void DisplaySoh2Report(ISI_REPORT *report)
{
int i;
ISI_SOH2 soh2;
ISI_REPORT_ENTRY *entry;
...
char buf[BUFLEN];
    ...
    for (i = 0; i < (int) report->nentry; i++) {
        entry = (ISI_REPORT_ENTRY *) &report->entry[i];
        if (entry->type != ISI_REPORT_TYPE_SOH2) {
            fprintf(stderr, "*** ERROR *** expected report type %d, but got %d!\n", ISI_REPORT_TYPE_SOH2, entry->type);
            exit(1);
        }
        isiUnpackSOH2(entry->payload, &soh2);
        printf("%7s ", sys.site);
        printf("%s ", utilTimeString(sys.tstamp.start, 108, buf, BUFLEN));
        printf("%s ", utilTimeString(sys.tstamp.write, 108, buf, BUFLEN));
        printf("%s ", isiSeqnoString2(&sys.seqno, buf));
...
static void soh2(char *server, ISI_PARAM *par)
{
ISI_REPORT *report;

    if ((report = isiReport(server, par, ISI_REPORT_TYPE_SOH2)) == NULL) {
        perror("isiReport");
        exit(1);
    }

    DisplaySoh2Report(report);
    isiFreeReport(report);
}
#endif
...
char *dbspec  = NULL;
    ...
    if (strcasecmp(req, "data") == 0) {
        ...
        SiteSpec = NULL;
        format = ISI_FORMAT_NATIVE;
    } else if (strcasecmp(req, "seed") == 0) {
        request = RAW;
        SiteSpec = NULL;
        format = ISI_FORMAT_MSEED;
    } else if (strncasecmp(req, "raw=", strlen("raw=")) == 0) {
        request = RAW;
        SiteSpec = req + strlen("raw=");
        if (strlen(SiteSpec) == 0) SiteSpec = NULL;
        format = ISI_FORMAT_NATIVE;
    } else if (strncasecmp(req, "seed=", strlen("seed=")) == 0) {
        request = RAW;
        SiteSpec = req + strlen("seed=");
        if (strlen(SiteSpec) == 0) SiteSpec = NULL;
        format = ISI_FORMAT_MSEED;
    ...
    switch (request) {
      case DATA: 
        ...
        break;
      case RAW: 
        if (VERBOSE) fprintf(stderr, "%s %s data request\n", server, format == ISI_FORMAT_MSEED ? "MiniSEED" : "raw");
#ifdef INCLUDE_DL_OPTION
        ...
        rawdata(server, &par, compress, format, &begseqno, &endseqno, SiteSpec);
        break;
      case CNF:
      ...
      case  WD:
        ...
        break;
      case  DL:
        if (VERBOSE) fprintf(stderr, "%s disk loop status request\n", server);
        dl(server, &par);
        break;
      default:
        help(argv[0]);
 ...
 *
 * $Log: main.c,v $
 * Revision 1.35  2015/12/08 18:49:57  dechavez
 * cast some UINT8 * to char * to calm DARWIN builds
 *
 * Revision 1.34  2015/12/07 18:47:09  dechavez
 * added some code to support printing ISI_SOH2 messages, but since they don't
 * exist and haven't even been defined yet the whole block is ifdef'd out
 *
 * Revision 1.33  2014/08/28 21:34:50  dechavez
 * really removed bud support
 *
 * Revision 1.32  2014/08/11 19:31:29  dechavez
 * removed BUD support
 *
 * Revision 1.31  2012/07/30 17:00:21  dechavez
 * increased LOCALBUFLEN to 16384
 *
 * Revision 1.30  2011/11/07 17:53:43  dechavez
 * BUD support
 *
 * Revision 1.29  2011/11/03 18:04:04  dechavez
 * support seqno based MiniSEED requests
 *
 * Revision 1.28  2011/07/28 00:00:22  dechavez
 * remamed raw() to rawdata() to avoid name clash now that curses.h is appearing in the includes
 *
 * Revision 1.27  2010/08/27 18:54:00  dechavez
