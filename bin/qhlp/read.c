#pragma ident "$Id: read.c,v 1.16 2015/12/09 18:42:28 dechavez Exp $"
/*======================================================================
 *
...
static void PauseForNewData(QDPLUS *handle, ISI_DL *input)
{
ISI_DL snapshot;
ISI_DL_SYS sys;
...
static char *fid = "ReadNextPacket";
    ...
    if (!qdpVerifyCRC(&pkt->qdp)) {
        LogMsg("%s: *** ERROR *** Oh shit. CRC error at index %lu (this should NEVER occur)!", fid, history.nxtndx);
        Exit(MY_MOD_ID + 3);
    }
...
/* If there isn't any preserved state, then configure to dump the entire QDP disk loop */
    ...
    if (qhlp->qdplus->state != QDPLUS_STATE_OK) {
        ...
        history.nxtndx = snapshot.sys->index.oldest;
        history.endndx = snapshot.sys->index.yngest;
        if (!VerifyIndicies(&snapshot)) Exit(MY_MOD_ID + 5);
...
/* We have state... find the range of packets to preload */
    ...
    if (isiIsUndefinedSeqno(&beg.seqno) || isiIsUndefinedSeqno(&end.seqno)) {
        ...
        history.nxtndx = snapshot.sys->index.oldest;
        history.endndx = snapshot.sys->index.yngest;
        if (!VerifyIndicies(&snapshot)) Exit(MY_MOD_ID + 7);
        qdplusSetLcqStateFlag(qhlp->qdplus, QDP_LCQ_STATE_PRODUCTION);
        LogMsg("going into production mode with nxtndx=0x%08x, endndx=0x%08x\n", history.nxtndx, history.endndx);
        return;
    }
...
/* We know where to find the data to backfill, set up reader accordingly */
    ...
    if (!VerifyIndicies(&snapshot)) Exit(MY_MOD_ID + 10);
 ...
 *
 * $Log: read.c,v $
 * Revision 1.16  2015/12/09 18:42:28  dechavez
 * various minor tweaks to get clean Mac OS X builds
 *
 * Revision 1.15  2011/10/31 17:10:06  dechavez
 * removed printfs that got trapped in accidental commit 1.14
 *
 * Revision 1.14  2011/10/31 17:06:29  dechavez
 * accidental commit, includes some printfs which need to get removed
 *
 * Revision 1.13  2010/03/22 21:54:50  dechavez
 * Removed all special handling of calibration monitor data
 *
 * Revision 1.12  2010/03/12 00:46:35  dechavez
 * rename CheckCalibrationStatus() to UpdateCalibrationState()
 *
 * Revision 1.11  2009/07/09 18:36:41  dechavez
 * switch to lcq instead of c1_stat for determining calibration state
 *
 * Revision 1.10  2009/07/08 16:02:11  dechavez
 * Fixed multiple Q330 automatic calibration detrigger bug, and verify CRC on all packets
 *
 * Revision 1.9  2009/07/02 23:08:18  dechavez
 * track calibration status while in RecoverState()
 *
