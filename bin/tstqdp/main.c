#pragma ident "$Id: main.c,v 1.16 2014/08/11 19:11:34 dechavez Exp $"
/*======================================================================
 *
...
/* The choices are only IDA10 or MiniSEED for the output, so we'll assume not IDA10 means MiniSEED. */
        if (par->format == IDA10) {
...
/* If we are giving the automaton a packet processor, then we just fire it up and let
 * the machine take care of all the packet handling details */

    if (par->func) {
        qdpSetUser(&par->q330, (void *) par, ProcessPacket);
        if ((qp = qdpConnect(&par->q330, par->lp, &errcode)) == NULL) {
...
/* Otherwise, we fire up the machine and then do the explicit packet handling ourselves */
...
"\n"
" Reboot example: %s q330=192.168.1.1:0123456789abcdef:0:0:0:0 reboot\n"
"   Data example: %s q330=192.168.1.1:0123456789abcdef:0:1:0:0 data\n"
"\n";

    fprintf(stderr,"usage: %s q330=host:serial:auth:sport:cport:debug [-func] command\n", myname);
    fprintf(stderr, "\n");
    fprintf(stderr, VerboseHelp, myname, myname);
    fprintf(stderr, "\n");
    exit(1);
...
printf("q330[%d]: port.link = %d\n", i, par->q330.port.link);
        par->lp = &lp;
 ...
 *
 * $Log: main.c,v $
 * Revision 1.16  2014/08/11 19:11:34  dechavez
 * A whole bunch of changes made back in 10/2013 as part of the rework of libqdp
 * and libqio to support data comm over the serial port.  See the 8/11/2014
 * version.c comments for either library (they are the same)
 *
