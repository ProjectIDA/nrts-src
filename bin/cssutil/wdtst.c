#pragma ident "$Id: wdtst.c,v 1.4 2014/08/11 19:27:36 dechavez Exp $"
/*======================================================================
 *
...
static int examine(struct cssio_wfdisc *wfdisc, struct stat *sbuf, long pos)
{
off_t len, eof, end;
...
static void process(FILE *fp, char *fname, char *myname)
{
...
struct stat sbuf;
    ...
    qsort((char *) wfdisc, nrec, sizeof(struct cssio_wfdisc), compare);
...
/* Check for internal consistency */
    ...
    for (total = 0, i = 0; i < nrec; i++) {
        ...
        if (errors) {
            printf("record number %ld rejected\n", i+1);
            ++total;
        } else {
 ...
 *
 * $Log: wdtst.c,v $
 * Revision 1.4  2014/08/11 19:27:36  dechavez
 * made more ansi compatible so that it would build under darwin
 *
