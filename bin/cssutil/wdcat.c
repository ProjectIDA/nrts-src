#pragma ident "$Id: wdcat.c,v 1.4 2015/12/08 20:48:20 dechavez Exp $"
/*======================================================================
 *
...
static void copy(FILE *wfp, struct cssio_wfdisc *out, struct cssio_wfdisc *in)
{
FILE *fp;
int wrdsize, status;
long i;
    ...
    for (i = 0; i < in->nsamp; i++) {
        ...
        if (status != wrdsize) {
            if (feof(fp)) {
                ...
                fprintf(stderr, "wdcat: %ld of %d samples written\n", i, in->nsamp);
                exit(5);
            } else {
...
static int continuous(struct cssio_wfdisc *prev, struct cssio_wfdisc *crnt)
{
...
static void wrtnewrec(FILE *ofp, struct cssio_wfdisc *newrec)
{
...
char *dir   = NULL;
char *dfile = NULL;
struct cssio_wfdisc *wfdisc, *prev, out;
...
/*  Read entire input wfdisc into memory and sort by station/channel/time  */
    ...
    qsort((char *) wfdisc, nrec, sizeof(struct cssio_wfdisc), wdcompare);
 ...
 *
 * $Log: wdcat.c,v $
 * Revision 1.4  2015/12/08 20:48:20  dechavez
