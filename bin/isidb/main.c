#pragma ident "$Id: main.c,v 1.6 2013/05/14 21:07:58 dechavez Exp $"
/*======================================================================
 *
...
UINT64 zerotime;
static char *imei = "300025010416330";
    ...
    if (isidbLookupSiteByIMEI(db, imei, site, net, &zerotime)) {
        printf("IMEI '%s' maps to site='%s', net='%s', zerotime=%s\n", imei, site, net, utilTimeString(zerotime, 1000, NULL, 0));
    } else {
        printf("no database entry for IMEI '%s'\n", imei);
    }

    if ((tmpstr = isidbLookupIMEIbySite(db, site)) != NULL) {
        printf("Site '%s' maps to IMEI '%s'\n", site, tmpstr);
    } else {
        printf("no IMEI database entry for site '%s'\n", site);
    }
 ...
 *
 * $Log: main.c,v $
 * Revision 1.6  2013/05/14 21:07:58  dechavez
 * test imei.map
 *
