...
char *arg;
    ...
    q330->cmd.code = Q330_CMD_TRIGINFO;
    ...
    for (i = 0; i < q330->cmd.arglst->count; i++) {
        ...
        if (q330->cmd.p32 == 0) {
            if (strcmp(arg, "0") == 0 || strcmp(arg, "1") == 0) {
                ...
                TriginfoHelp(q330->cmd.name, 0, NULL);
            }
...
QDP_DP_TOKEN token;
    ...
    printf("\n");
    qdpPrintTokenDetList(stdout, &token.det);
    printf("\n");
    qdpPrintTokenCdsList(stdout, &token.cds);
    printf("\n");
 ...
 * corrected error message printed when qdpRequestTriginfo() fails
 *
 * Revision 1.2  2009/09/28 22:15:20  dechavez
 * use qdpUnpackTriginfo() instead of explicit request and unpack calls
 *
