#pragma ident "$Id: main.c,v 1.10 2015/12/07 19:55:33 dechavez Exp $"
/*======================================================================
 *
...
char buffer[2 * IDA10_MAXRECLEN];
    ...
    fprintf(stderr, "%s - %s (build: %s %s)\n", argv[0], VersionIdentString, __DATE__, __TIME__);

    Buffer = buffer;
    init(argc, argv, &count, &verbose, &gz);
    while (rdrec(gz, &ts, &count, verbose)) {
        wrtdat(&ts, &count);
 ...
 *
 * $Log: main.c,v $
 * Revision 1.10  2015/12/07 19:55:33  dechavez
 * all sorts of changes to calm OS X builds
 *
 * Revision 1.9  2011/02/24 17:22:39  dechavez
