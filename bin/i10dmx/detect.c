#pragma ident "$Id: detect.c,v 1.5 2015/12/07 19:55:33 dechavez Exp $"
/*======================================================================
 *
...
LNKLST_NODE *crnt;
    ...
    while (crnt != NULL) {
        entry = (LIST_ENTRY_ *) crnt->payload;
...
DETECTOR *result;
    ...
    if (!listAppend(head, &new, sizeof(LIST_ENTRY_))) {
...
void InitDetector(Q330_DETECTOR *pDetector)
{
    detector = pDetector;
    ...
    switch (detector->engine.type) {
      case DETECTOR_TYPE_STALTA:
        printf("STA/LTA event detector: %s %d %u %u %u %u %.2f %.2f\n",
            detector->channels,
            detector->votes,
            detector->pre,
            detector->pst,
      ...
      default:
        printf("UNEXPECTED TYPE %d event detector: %s = %d %u %u\n",
 ...
 *
 * Revision 1.4  2012/08/02 17:28:17  dechavez
 * changed LIST_ENTRY to LIST_ENTRY_ to avoid WIN32 clashes (aap)
 *
 * Revision 1.3  2010/12/17 19:49:39  dechavez
 * 2.4.1
 *
