#pragma ident "$Id: rdrec.c,v 1.16 2015/12/07 19:55:33 dechavez Exp $"
/*======================================================================
 *
...
static char tmpbuf[256];
    ...
    while (1) {
        nread = ida10ReadGz(gz, buf, IDA10_MAXRECLEN, &RecordType, FALSE);
        if (nread < 0) {
            status = nread;
        ...
        if (RecordType == IDA10_TYPE_LM) {
            ...
            sprintf(Buffer, "rdrec: unknown record no. %u dropped\n", count->rec);
            logmsg(Buffer);
            continue;
        }
...
//            if (needToSwap) ida10ReorderData(ts);
            ...
            sprintf(Buffer, "rdrec: can't decode TS record no. %u\n", count->rec
            );
            logmsg(Buffer);
        }
    }
 ...
 *
 * $Log: rdrec.c,v $
 * Revision 1.16  2015/12/07 19:55:33  dechavez
 * all sorts of changes to calm OS X builds
 *
 * Revision 1.15  2011/03/17 20:25:56  dechavez
 * support change in return value of ida10ReadGz (from error code to nbytes, libida10 2.12.0)
 *
 * Revision 1.14  2011/02/25 19:38:33  dechavez
 * added TSheader (constant FALSE) to ida10ReadGz() parameters
 *
 * Revision 1.13  2011/02/24 17:23:46  dechavez
 * use ida10ReadGz() for input
 *
