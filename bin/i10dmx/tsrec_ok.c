#pragma ident "$Id: tsrec_ok.c,v 1.4 2015/12/07 19:55:33 dechavez Exp $"
/*======================================================================
 *
...
int tsrec_ok(IDA10_TS *ts, struct counter *count)
{
    ...
    if (ts->hdr.nsamp < 0) {
        ...
        sprintf(Buffer+strlen(Buffer),"Too large nsamp (%d), ", ts->hdr.nsamp);
        sprintf(Buffer+strlen(Buffer),"record %d ignored.\n",count->rec);
        logmsg(Buffer);
        return FALSE;
    }
 ...
 *
 * $Log: tsrec_ok.c,v $
 * Revision 1.4  2015/12/07 19:55:33  dechavez
