#pragma ident "$Id: nrts.c,v 1.2 2015/12/23 22:51:04 dechavez Exp $"
/*======================================================================
 *
 * NRTS display - should never get here if qmon->mon is NULL
 *
 *====================================================================*/
#include "qmon.h"

#define LABEL " NRTS Disk Loop Status "
...
WINDOW *win;
    ...
    PrintHostTime(win, 0);
    isidlMonUpdateNRTS(win, qmon->mon, StartingRow);
    wrefresh(win);
    return;
}
...
/* At this point we have a name, hopefully of the form siteXX. Truncate it. */
    ...
    isidlSetMonNRTS(qmon->mon, target);
}
...
int i, row, col;
    ...
    col = QMON_NRTS_COLS/2 - strlen(LABEL)/2;
    mvwprintw(win, row++, col, LABEL);
    StartingRow = row;

    isidlMonStaticNRTS(win, StartingRow);
