/* $Id: platform.h,v 1.48 2015/12/03 17:49:36 dechavez Exp $ */
/*====================================================================
 *
...
#ifdef HAVE_MSYNC
#   define MSYNC(addr, len) msync(addr, len, MS_ASYNC)
#elif defined WIN32
...
#   include "mysem.h"
    typedef MYSEM SEMAPHORE;
#   define SEM_INIT(id, init, max) mysemInit((id), init)
#   define SEM_POST(id)            mysemPost(id)
#   define SEM_WAIT(id)            mysemWait(id)
#   define SEM_TRYWAIT(id)         mysemTryWait(id)
#   define SEM_DESTROY(id)         mysemDestroy(id)
...
#ifndef UNKNOWN_BYTE_ORDER
#define UNKNOWN_BYTE_ORDER    2
#endif

#if defined(X86_16BIT) || defined(X86_WIN32) || defined(X86_UNIX)
#   ifndef LTL_ENDIAN_HOST
#       define LTL_ENDIAN_HOST
#   endif
#   define NATIVE_BYTE_ORDER LTL_ENDIAN_BYTE_ORDER
#elif defined(SPARC_UNIX)
...
#ifdef SOLARIS
#define ASCTIME_R(tm, buf, buflen) asctime_r(tm, buf, buflen)
#else
#define ASCTIME_R(tm, buf, buflen) asctime_r(tm, buf)
#endif
 ...
 *
 * $Log: platform.h,v $
 * Revision 1.48  2015/12/03 17:49:36  dechavez
 * Gag, OS X lackes anonymous symaphores, so added DARWIN macros to use the
 * local implementation from libutil.a mysem.c
 *
 * Revision 1.47  2015/11/13 20:56:13  dechavez
 * include sys/ioctl.h if DARWIN
 *
 * Revision 1.46  2014/08/08 15:38:31  dechavez
 * changed last argument of SENDTO and RECVFROM macros from int * to socket_t *
 *
 * Revision 1.45  2014/04/15 20:50:45  dechavez
 * shuffled the order of some SOLARIS includes (don't recall why)
 *
 * Revision 1.44  2012/08/02 17:43:09  dechavez
 * added WIN32 EOVERFLOW if not already defined (aap)
 *
 * Revision 1.43  2012/02/14 19:41:52  dechavez
 * Replaced *_UNIX32 with *_UNIX (32-bit flag was not needed).
 * Assume X86_UNIX if sparc is not defined (because I couldn't find a
 * reliable define that flagged X86 systems)
 *
 * Revision 1.42  2010/06/23 16:09:36  dechavez
 * asynchronous MSYNC (MS_ASYNC)
 *
 * Revision 1.41  2010/06/14 19:26:16  dechavez
 * added HAVE_INTTYPES for LINUX
 *
 * Revision 1.40  2010/04/12 16:17:44  dechavez
 * added HAVE_FORK for unix platforms
 *
 * Revision 1.39  2010/02/08 19:02:26  dechavez
 * added HAVE_RLIMIT and associated include file for applicable platforms
 *
 * Revision 1.38  2009/10/02 19:52:54  dechavez
 * added ASCTIME_R macro
 *
