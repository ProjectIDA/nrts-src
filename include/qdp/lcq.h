#pragma ident "$Id: lcq.h,v 1.19 2017/01/25 18:37:51 dechavez Exp $"
/*======================================================================
 *
...
#define QDP_SP_TYPE_CALBEG    0   /* calibration start */
#define QDP_SP_TYPE_CALEND    1   /* calibration abort */
#define QDP_SP_TYPE_CNPBLK    2   /* CNP Block data */
#define QDP_SP_TYPE_CFGBLK    3   /* Configuration Blockette */
#define QDP_SP_TYPE_ENVDAT    4   /* Environmental Processor Data */

/* Possible config blockettes for QDP_SP_CFGBLK */

#define QDP_CFG_FIX    1
#define QDP_CFG_GLOB   2
#define QDP_CFG_AUXAD  3
#define QDP_CFG_SS1    4
#define QDP_CFG_SS2    5
...
/* type = QDP_SP_TYPE_CALBEG */

typedef struct {
    UINT8 channel;       /* "channel" (undocumented?) */
...
#define QDP_SP_NCOUPLE 12
    UINT8  coupling[QDP_SP_NCOUPLE]; /* coupling bytes */
} QDP_SP_CALBEG;
...
typedef struct {
    UINT8 channel;       /* "channel" (undocumented?) */
    UINT8 flags;         /* blockette flags */
    UINT16 size;         /* blockette size */
    UINT8 port;          /* port number */
    UINT8 unitid;        /* unit id */
    UINT8 status;        /* device specific status */
...
typedef struct {
    ...
    QDP_TYPE_C1_FIX fix;   /* data for type == QDP_CFG_FIX */
    QDP_TYPE_C1_GLOB glob; /* data for type == QDP_CFG_GLOB */
    QDP_TYPE_AUXADC auxad; /* data for type == QDP_CFG_AUXAD */
    QDP_TYPE_SSCFG ss1;    /* data for type == QDP_CFG_SS1 */
    QDP_TYPE_SSCFG ss2;    /* data for type == QDP_CFG_SS2 */
} QDP_SP_CFGBLK;

typedef struct {
    UINT32 seqno;          /* data record sequence number, imported from DT_DATA header */
    UINT8 fc;              /* "channel" = $FC */
    QDP_EPCHN channel;     /* EP channel */
    UINT16 size;           /* blockette size */
    UINT64 nsint;          /* sample interval in nanoseconds */
    /* use this if size == 8 */
    INT32 value;           /* sample value */
    /* use these if size != 8 */
    INT32 prev;            /* uncompressed previous sample */
    UINT16 doff;           /* offset to start of compressed data */
    UINT8 *map;            /* variable length Steim 2 compression map */
...
typedef struct {
    UINT32 seqno;    /* data record sequence number, imported from DT_DATA header */
    UINT8  type;     /* event type (QDP_SP_xxx) */
    union {
        QDP_SP_CALBEG calbeg;
        QDP_SP_CALEND calend;
        QDP_SP_CNPBLK cnpblk;
        QDP_SP_CFGBLK cfgblk;
        QDP_SP_ENVDAT envdat;
    } data;
} QDP_BLOCKETTE_SPEC;
...
typedef struct {
    UINT32 seqno;
    LNKLST blist; /* linked list of one or more QDP_TYPE_DT_BLOCKETTEs */
...
typedef struct {
    ...
    BOOL cal_in_progress; /* TRUE when calibration is active in current packet */
    UINT8 src[QDP_LCQSRC_LEN]; /* LCQ source parameters */
} QDP_HLP;
...
#define QDP_HLP_RULE_FLAG_NOHLP_1SECHK 0x00000008 /* if set, do NOT test for and discard 1-sec overlaps qdpBuildHLP() */
    UINT32 flags;      /* option flags */
} QDP_HLP_RULES;

#define QDP_DEFAULT_HLP_RULE_FLAG QDP_HLP_RULE_FLAG_STRICT
#define QDP_DEFAULT_HLP_RULES {0, 0, NULL, NULL, FALSE, QDP_DEFAULT_HLP_RULE_FLAG}
...
#define QDP_LCQ_EVENT_CALBEG       0x0040 /* QDP_SP_CALBEG special packet received */
#define QDP_LCQ_EVENT_CALEND       0x0080 /* QDP_SP_CALEND special packet received */
#define QDP_LCQ_EVENT_CNPBLK       0x0100 /* QDP_SP_CNPBLK special packet received */
#define QDP_LCQ_EVENT_CFGBLK       0x0200 /* QDP_SP_CFGBLK special packet received */
#define QDP_LCQ_EVENT_ENVDAT       0x0400 /* QDP_SP_ENVDAT special packet received */
 ...
 *
 * $Log: lcq.h,v $
 * Revision 1.19  2017/01/25 18:37:51  dechavez
 * added QDP_HLP_RULE_FLAG_NOHLP_1SECHK
 *
 * Revision 1.18  2016/01/23 00:07:00  dechavez
 * changed QDP_SP_ENVDAT channel type to QDP_EPCHN
 *
 * Revision 1.17  2016/01/19 22:48:58  dechavez
 * initial environmental processor support
 *
 * Revision 1.16  2013/01/14 22:32:02  dechavez
 * made QDP_DEFAULT_HLP_RULE_FLAG QDP_HLP_RULE_FLAG_STRICT
 *
 * Revision 1.15  2010/03/12 00:22:59  dechavez
 * added LCQ src parameters to QDP_HLP
 *
 * Revision 1.14  2009/10/29 17:01:34  dechavez
 * removed obsolete reqtoken from QDP_HLP_RULES (should be using flags, instead)
 * and modified QDP_DEFAULT_HLP_RULES accordingly.   Added space to hold decoded
 * configuration blockettes in QDP_BLOCKETTE_SPEC.
 *
 * Revision 1.13  2009/10/20 22:23:36  dechavez
 * QDP_CFG_x blockette identifiers, plus QDP_SP_ENVDAT and QDP_LCQ_EVENT_ENVDAT
 *
 * Revision 1.12  2009/07/09 18:13:42  dechavez
 * added support for special packets, added cal_in_progress to QDP_HLP
 *
